...
- Listas podem ser limitadas para exibição. Para totais, use sempre os campos exactTotals ou os agregados do summary360.
- Perguntas hipotéticas como “e se pagar metade?” são cálculos/simulações, não pedidos de alteração de dados.
...
async function summary360(databaseId: number) {
  ...
  const [clients, loans, payments, cash, financings, vehicles, sales, products, agents] = await Promise.all([
    ...
    sql`SELECT COUNT(*)::int count,
      ...
      COALESCE(SUM(CAST(amount AS numeric)) FILTER(WHERE status<>'pago' AND status<>'atrasado' AND "dueDate"::date>=CURRENT_DATE),0) pending,
...
async function client360(databaseId: number, search: string) {
  ...
  const [loans, payments, interestHistory, financings, vehicles, sales, cash, totals] = await Promise.all([
    ...
    sql`SELECT
      (SELECT COALESCE(SUM(CAST("remainingBalance" AS numeric)),0) FROM loans WHERE "databaseId"=${databaseId} AND "clientId"=ANY(${ids})) "loanRemaining",
      (SELECT COALESCE(SUM(CAST("remainingBalance" AS numeric)),0) FROM "vehicleFinancings" WHERE "databaseId"=${databaseId} AND "clientId"=ANY(${ids})) "financingRemaining",
      (SELECT COALESCE(SUM(CAST(p.amount AS numeric)),0) FROM payments p LEFT JOIN loans l ON l.id=p."loanId" LEFT JOIN "vehicleFinancings" vf ON vf.id=p."vehicleFinancingId" WHERE p."databaseId"=${databaseId} AND p.status='pago' AND (l."clientId"=ANY(${ids}) OR vf."clientId"=ANY(${ids}))) "paid",
      (SELECT COALESCE(SUM(CAST(p.amount AS numeric)),0) FROM payments p LEFT JOIN loans l ON l.id=p."loanId" LEFT JOIN "vehicleFinancings" vf ON vf.id=p."vehicleFinancingId" WHERE p."databaseId"=${databaseId} AND (p.status='atrasado' OR (p.status<>'pago' AND p."dueDate"::date<CURRENT_DATE)) AND (l."clientId"=ANY(${ids}) OR vf."clientId"=ANY(${ids}))) "overdue"`,
  ]);
  return { search, clients, exactTotals: totals[0], loans, payments, interestHistory, financings, vehicles, vehicleSales: sales, cashFlow: cash };
}
...
async function paymentContext(databaseId: number, search: string, text: string) {
  ...
  const [rows, aggregate] = await Promise.all([
    ...
    sql`
      SELECT COUNT(*)::int count,
        COALESCE(SUM(CAST(p.amount AS numeric)),0) total,
        COALESCE(SUM(CAST(p.amount AS numeric)) FILTER(WHERE p.status='pago'),0) paid,
        COALESCE(SUM(CAST(p.amount AS numeric)) FILTER(WHERE p.status='atrasado' OR (p.status<>'pago' AND p."dueDate"::date<CURRENT_DATE)),0) overdue,
        COALESCE(SUM(CAST(p.amount AS numeric)) FILTER(WHERE p.status<>'pago' AND p.status<>'atrasado' AND p."dueDate"::date>=CURRENT_DATE),0) pending
      FROM payments p
  ...
  return { search: search || null, period, exactTotals: aggregate[0], rows };
}
...
async function cashContext(databaseId: number, search: string, text: string) {
  ...
  const entries = Number((aggregate[0] as any)?.entries || 0);
  const exits = Number((aggregate[0] as any)?.exits || 0);
  return { search: search || null, period, exactTotals: { ...(aggregate[0] as any), net: Math.round((entries - exits) * 100) / 100 }, rows };
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const asksChange = !hypothetical && (
      ...
      || /\b(dar baixa|de baixa|marque como pago)\b/.test(text)
    );
