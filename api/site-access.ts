...
function ensureAccessTable() {
  ...
  accessTablePromise = (async () => {
    ...
    `;
    await sql`
      CREATE INDEX IF NOT EXISTS site_access_logs_created_idx
      ON site_access_logs ("createdAt" DESC)
    `;
    await sql`
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const referrer = String(body?.referrer ?? "").trim().slice(0, 1000) || null;
    const userAgent = String(req?.headers?.["user-agent"] ?? "").slice(0, 2000) || null;
    const forwarded = String(req?.headers?.["x-forwarded-for"] ?? "");
    const ipAddress = (forwarded.split(",")[0]?.trim() || String(req?.socket?.remoteAddress ?? ""))
      .slice(0, 120) || null;
    ...
    if (token) {
      const session = await sql`
        SELECT u.id
         ...
         LIMIT 1
      `;
      if (session[0]?.id) userId = Number(session[0].id);
    }

    await sql`
      INSERT INTO site_access_logs (
        "userId", path, referrer, "ipAddress", "userAgent", "createdAt"
      ) VALUES (
        ${userId}, ${path}, ${referrer}, ${ipAddress}, ${userAgent}, NOW()
      )
    `;

    return sendJson(res, 201, { success: true });
  } catch (error) {
    console.error("[site-access]", error);
