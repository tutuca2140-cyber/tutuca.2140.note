...
- Continue o assunto usando o histórico.
...
- Em cálculos, priorize valores determinísticos fornecidos pelo sistema quando existirem.

NOTE NOTE
- Use somente o contexto operacional autorizado recebido.
- Os módulos podem incluir clientes, empréstimos, histórico de juros, pagamentos, caixa, veículos, financiamentos, produtos, vendas e agentes.
...
async function getDashboardContext(databaseId: number) {
  ...
  const [clients, loans, payments, cash, financings] = await Promise.all([
    ...
    sql`SELECT
      COALESCE(SUM(CASE WHEN type='ENTRADA' THEN CAST(amount AS numeric) ELSE 0 END),0) AS entries,
  ...
  const value = { clients: clients[0], loans: loans[0], payments: payments[0], cash: cash[0], financings: financings[0] };
  cacheSet(key, value);
  return value;
}

async function getOperationalContext(databaseId: number, intent: Intent, message: string) {
  const sql = getSql();
  ...
  if (intent === "loan") {
    const clients = search ? await sql`SELECT id,name FROM clients WHERE "databaseId"=${databaseId} AND name ILIKE ${like} LIMIT 20` : [];
  ...
  if (intent === "payment") {
    const rows = await sql`SELECT p.*, c.name AS "clientName"
      FROM payments p
  ...
  if (intent === "vehicle") {
    const rows = await sql`SELECT * FROM vehicles WHERE "databaseId"=${databaseId} ORDER BY "updatedAt" DESC LIMIT 80`;
  ...
  if (intent === "financing") {
    const rows = await sql`SELECT vf.*, c.name AS "clientName" FROM "vehicleFinancings" vf
  ...
  if (intent === "product") {
    const products = await sql`SELECT * FROM products WHERE "databaseId"=${databaseId} ORDER BY "updatedAt" DESC LIMIT 80`;
  ...
  if (intent === "agent") {
    const rows = await sql`SELECT * FROM agents WHERE "databaseId"=${databaseId} ORDER BY "updatedAt" DESC LIMIT 80`;
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const canReadOperational = context.user.role === "super_admin" || (context.user.canView === true && context.user.dashboardOnly !== true);
    const dashboardOnly = context.user.dashboardOnly === true && context.user.canView === true;

    let operationalContext: unknown = { access: "general_knowledge_only" };
    if (canReadOperational) {
      operationalContext = intent === "general"
        ? { dashboard: await getDashboardContext(context.database.id) }
        : await getOperationalContext(context.database.id, intent, message);
    } else if (dashboardOnly) {
      operationalContext = { access: "dashboard_only", dashboard: await getDashboardContext(context.database.id) };
    }

    const [history, deterministic] = await Promise.all([
      getRecentConversation(context.user.id, context.database.id),
      Promise.resolve(deterministicMath(message)),
    ]);
    ...
    const response = await fetch("https://ai-gateway.vercel.sh/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "system",
            content: `Usuário: ${context.user.name || context.user.username || "Usuário"}. Papel: ${context.user.role}. Banco: ${context.database.name}. Intenção detectada: ${intent}. Dados autorizados relevantes: ${JSON.stringify(operationalContext)}. Cálculo determinístico, se aplicável: ${JSON.stringify(deterministic)}.`,
          },
    ...
    return sendJson(res, 200, { reply, intent });
  } catch (error) {
