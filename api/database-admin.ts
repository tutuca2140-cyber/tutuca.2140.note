...
function canAccessDatabase(
  user: AdminUser,
  accessRows: Array<{ databaseId: number }>,
  databaseId: number
) {
...
function remapId(value: unknown, map: IdMap | undefined, required = false) {
  ...
  if (required) {
    throw new Error("Não foi possível reconstruir um vínculo interno da cópia.");
  }
  return null;
}
...
async function duplicateDatabase(
  ...
  if (!canAccessDatabase(user, accessRows, sourceDatabaseId)) {
    throw Object.assign(new Error("Você não tem acesso a este banco de dados."), {
      statusCode: 403,
    });
  }
  if (user.role === "admin" && accessRows.length >= 3) {
    throw Object.assign(
      new Error(
        "Este administrador já está vinculado ao limite de três bancos. Ajuste os vínculos antes de duplicar."
      ),
      { statusCode: 400 }
    );
  }
  ...
  if (!sourceDatabase) {
    throw Object.assign(new Error("Banco de dados não encontrado."), {
      statusCode: 404,
    });
  }
  ...
  try {
    ...
    const agents = await cloneTable(
      client,
      "agents",
      sourceDatabaseId,
      targetDatabaseId
    );
    const clients = await cloneTable(
      client,
      "clients",
      sourceDatabaseId,
      targetDatabaseId,
    ...
    const vehicles = await cloneTable(
      client,
      "vehicles",
      sourceDatabaseId,
      targetDatabaseId
    );
    const products = await cloneTable(
      client,
      "products",
      sourceDatabaseId,
      targetDatabaseId
    );
    const loans = await cloneTable(
      client,
      "loans",
      sourceDatabaseId,
      targetDatabaseId,
    ...
    const financings = await cloneTable(
      client,
      "vehicleFinancings",
      sourceDatabaseId,
      targetDatabaseId,
    ...
    const payments = await cloneTable(
      client,
      "payments",
      sourceDatabaseId,
      targetDatabaseId,
    ...
    const cashFlow = await cloneTable(
      client,
      "cash_flow",
      sourceDatabaseId,
      targetDatabaseId,
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!canAccessDatabase(user, accessRows, databaseId)) {
      return sendJson(res, 403, {
        success: false,
        message: "Você não tem acesso a este banco de dados.",
      });
    }
    ...
    if (action === "duplicate") {
      ...
      if (!name || name.length > 255) {
        return sendJson(res, 400, {
          success: false,
          message: "Informe um nome válido para a cópia.",
        });
      }
      const duplicated = await duplicateDatabase(
        client,
        user,
        databaseId,
        name,
