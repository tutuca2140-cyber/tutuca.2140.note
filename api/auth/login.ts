...
import {
  getSql, makeSessionToken, readJsonBody, sendJson, setSessionCookie,
} from "./_shared.js";
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!username || !password) {
      return sendJson(res, 400, { success: false, message: "Informe usuário e senha." });
    }

    const sql = getSql();
    const rows = await sql`
      SELECT
        id, username, "passwordHash", name, email, role,
    ...
    if (!user.isActive) {
      return sendJson(res, 403, { success: false, message: "Usuário desativado." });
    }
    ...
    return sendJson(res, 200, {
      ...
      user: {
        ...
        name: user.name,
        email: user.email,
        role: isDraco ? "super_admin" : user.role,
      },
    });
  } catch (error) {
    console.error("[auth/login]", error);
    return sendJson(res, 500, { success: false, message: "Não foi possível realizar o login." });
  }
}
