...
export default function Emprestimos() {
  ...
  const plan = useMemo(() => {
    ...
    const periods =
      form.installments.trim() === "" ? 1 : Number(form.installments);
    if (
  ...
  const filteredLoans = (loans ?? []).filter(loan =>
    `${clientMap.get(loan.clientId) ?? ""} ${loan.description ?? ""} ${loan.status}`
      .toLowerCase()
  ...
  const openEdit = (loan: NonNullable<typeof loans>[number]) => {
    ...
    setForm({
      ...
      endDate: dateInput(loan.endDate),
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!plan || !form.clientId) {
      toast.error(
        "Selecione um cliente e informe valores válidos para calcular o contrato."
      );
      return;
    }
    try {
      const payload = {
        ...
        installmentAmount: form.installmentAmount.trim() || undefined,
      ...
      if (editingId !== null) {
        ...
        toast.success(
          "Empréstimo lançado com sucesso. A saída foi registrada no caixa."
        );
      }
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Contratos, saldos devedores e histórico financeiro do banco ativo.
            </p>
          </div>
          {user?.canInsert && (
          ...
          <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingId === null
                  ? "Novo empréstimo"
                  : `Editar empréstimo #${editingId}`}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  <Label htmlFor="loan-client">Cliente *</Label>
                  <select
                    id="loan-client"
                    value={form.clientId}
                    onChange={event =>
                ...
                <div>
                  <Label htmlFor="loan-amount">Principal (R$) *</Label>
                  <Input
                    id="loan-amount"
                    type="number"
                    min="0.01"
                ...
                <div>
                  <Label htmlFor="loan-rate">Taxa por período (%) *</Label>
                  <Input
                    id="loan-rate"
                    type="number"
                    min="0"
                ...
                <div>
                  <Label htmlFor="loan-period">Periodicidade da taxa</Label>
                  <select
                    id="loan-period"
                    ...
                    onChange={event =>
                      updateField(
                        "ratePeriod",
                        event.target.value as RatePeriod
                      )
                    }
                    className="mt-2 h-10 w-full rounded-md border bg-background px-3 text-sm"
                  >
                ...
                <div>
                  <Label htmlFor="loan-installments">
                    Quantidade de períodos (opcional)
                  </Label>
                  <Input
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={event =>
                      updateField("installmentAmount", event.target.value)
                    }
                    placeholder={
                      plan ? plan.installmentAmount.toFixed(2) : "0,00"
                    }
                  />
                </div>
                <div>
                  <Label htmlFor="loan-start">Data do empréstimo *</Label>
                  <Input
                    id="loan-start"
                    type="date"
                ...
                <div>
                  <Label htmlFor="loan-end">Vencimento (opcional)</Label>
                  <Input
                    id="loan-end"
                    type="date"
                    value={form.endDate}
                ...
                <div className="sm:col-span-2">
                  <Label htmlFor="loan-description">Observações</Label>
                  <Textarea
                    id="loan-description"
                    ...
                    placeholder="Garantias, condições ou observações"
                  />
                </div>
              </div>
                ...
                {plan ? (
                  <div className="grid gap-3 text-sm sm:grid-cols-4">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        {currency(plan.totalAmount)}
                      </p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Parcela</span>
                      <p className="font-semibold text-primary">
                        {currency(plan.installmentAmount)}
                      </p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Regra</span>
                      <p className="font-semibold">Saldo × taxa</p>
                    </div>
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">
                    Informe principal e taxa para visualizar a prévia.
                  </p>
                )}
              </div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      ...
                      {loan.interestType === "compound"
                        ? "Juros compostos"
                        : "Juros simples"}{" "}
                      · {Number(loan.interestRate).toLocaleString("pt-BR")} %/
                      {loan.ratePeriod === "month" ? "mês" : loan.ratePeriod}
                    </p>
                  </div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
                    <div>
                      ...
                      <p className="font-semibold">{currency(loan.amount)}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Total pago</span>
                      <p className="font-semibold">
                        {currency(loan.totalPaid)}
                      </p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Juros acum.</span>
                      <p className="font-semibold">
                        {currency(loan.accruedInterest)}
                      </p>
                    </div>
                    <div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2 border-t pt-4">
                    ...
                    {user?.canInsert && (
                      <Button size="sm" variant="outline" asChild>
                        <a href={`/pagamentos?loanId=${loan.id}`}>Pagamento</a>
                      </Button>
                    )}
                    {canDelete && (
                      <Button
        ...
        >
          <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Detalhes do empréstimo #{detailId}</DialogTitle>
            </DialogHeader>
              ...
              <div className="space-y-5">
                <div className="grid gap-3 sm:grid-cols-4">
                  ...
                  <div className="rounded-lg bg-muted/50 p-3">
                    ...
                    <p className="font-semibold">
                      {currency(details.loan.accruedInterest)}
                    </p>
                  </div>
                  <div className="rounded-lg bg-primary/10 p-3">
                    ...
                    <p className="font-semibold text-primary">
                      {currency(details.loan.remainingBalance)}
                    </p>
                  </div>
                </div>
                <div>
                  <h3 className="mb-2 font-semibold">Histórico de juros</h3>
                  {details.interestHistory.length ? (
                    <div className="space-y-2">
                      {details.interestHistory.map(item => (
                        <div
                          key={item.id}
                          className="flex flex-wrap justify-between gap-2 rounded-lg border p-3 text-sm"
                        >
                          <span>
                            {item.periodReference === "CONTRATO_INICIAL"
                              ? "Juros do contrato"
                              : item.periodReference}
                          </span>
                          <span>
                            Gerado: {currency(item.interestGenerated)}
                          </span>
                          <span>
                            Saldo: {currency(item.updatedPrincipalBalance)}
                          </span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-2">
                      {details.payments.map(payment => (
                        ...
                        >
                          ...
                          <span>Pago: {currency(payment.amount)}</span>
                          <span>Juros: {currency(payment.interestAmount)}</span>
                          <span>
                            Amortização: {currency(payment.principalAmount)}
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <div>
                  ...
                  {details.cashFlow.length ? (
                    <div className="space-y-2">
                      {details.cashFlow.map(movement => (
                        <div
                          key={movement.id}
                          className="flex flex-wrap justify-between gap-2 rounded-lg border p-3 text-sm"
                        >
