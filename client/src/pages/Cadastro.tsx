...
function readBillingMethod(): BillingMethod {
  ...
  try {
    if (sessionStorage.getItem("notenote:selected-billing") === "pix_annual")
      return "pix_annual";
  } catch {}
  return "card_monthly";
}
...
export default function Cadastro() {
  ...
  const [trialCancellationAccepted, setTrialCancellationAccepted] =
    useState(false);
  const [marketingOptIn, setMarketingOptIn] = useState(false);

  const refreshCaptcha = useCallback(async () => {
    try {
      const r = await fetch("/api/auth/captcha", { cache: "no-store" });
      const j = await r.json();
      if (r.ok && j?.success)
        setCaptcha({ question: j.question, token: j.token });
    } catch {}
  }, []);
  useEffect(() => {
    void refreshCaptcha();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!fullNameValid)
      return setError("Informe seu nome e sobrenome completos.");
    if (!whatsappValid)
      return setError(
    ...
    if (!legalAccepted)
      return setError(
        "Aceite os Termos de Uso, a Política de Privacidade e as regras do teste gratuito/cancelamento para continuar."
      );
    if (!notRobot || !captchaAnswer.trim())
      return setError(
        "Confirme que você não é um robô e resolva a verificação."
      );
    setLoading(true);
    try {
      const r = await fetch("/api/auth/register-commercial", {
        method: "POST",
      ...
      if (billingMethod === "pix_annual") {
        ...
        if (!pix?.qrCode)
          throw new Error(
            "Cadastro criado, mas o Asaas não retornou o QR Code Pix."
          );
      ...
      if (!paymentUrl.startsWith("https://"))
        throw new Error(
          "Cadastro criado, mas o Asaas não retornou o link de pagamento."
        );
      setCheckoutUrl(paymentUrl);
      setSuccess(true);
          ...
          <Card className="border-emerald-200 shadow-xl">
            <CardContent className="p-8 text-center">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-600">
                <strong>{name.trim()}</strong>, você escolheu o plano{" "}
              ...
              {isFreePlan ? (
                ...
                <div className="mt-6 text-left">
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
                    ...
                    Consulte as condições vigentes de pagamento e período
                    contratado.
                  </div>
                  <div className="mt-5 rounded-2xl border bg-white p-5 text-center shadow-sm">
                    <div className="mx-auto inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-extrabold uppercase tracking-wide text-emerald-700">
                    ...
                    {pixInfo.qrCodeBase64 && (
                      <img
                        ...
                        className="mx-auto mt-5 h-56 w-56 rounded-xl border bg-white p-2"
                      />
                    )}
                    <Label htmlFor="pix-copy" className="mt-5 block text-left">
                      Pix Copia e Cola
                    </Label>
                    <textarea
                      id="pix-copy"
                      ...
                      className="mt-2 h-24 w-full resize-none rounded-xl border bg-slate-50 p-3 text-xs text-slate-700"
                    />
                    <Button
                      type="button"
                      onClick={copyPix}
                    ...
                    {checkoutUrl.startsWith("https://") && (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="mt-3 inline-flex h-11 w-full items-center justify-center rounded-xl border border-emerald-200 px-5 font-bold text-emerald-700"
                      >
                ...
                <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50 p-4 text-left text-sm text-blue-950">
                  Você tem 7 dias grátis. A primeira cobrança mensal no cartão
                  está programada para depois do período de teste, conforme as
                  condições aceitas.
                </div>
              )}
              {!isFreePlan &&
          ...
          <CardContent>
            {plan ? (
              ...
              <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                ...
                <Link href="/planos">
                  <a className="font-bold underline">Escolha Basic ou Plus</a>
                </Link>
                .
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {!isFreePlan && billingMethod === "pix_annual" && (
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Utilizado pelo Asaas para gerar o Pix.
                  </p>
                </div>
              )}
              <div>
              ...
              <div className="space-y-3 rounded-2xl border border-blue-100 bg-blue-50/60 p-4">
                ...
                <div className="border-t border-blue-100 pt-3">
                  <label className="flex items-start gap-3 text-sm">
                    ...
                    />
                    <span>
                      <strong>Opcional:</strong> autorizo o Note Note a enviar
                      para meu e-mail novidades, ofertas, promoções, conteúdos e
                      comunicações publicitárias. Posso retirar essa autorização
                      posteriormente.
                    </span>
                  </label>
                </div>
                <p className="text-xs text-slate-500">
                  O Note Note registra a versão dos documentos, data e hora,
                  plano, cobrança, escolha de marketing e informações técnicas
                  necessárias para comprovar o aceite.
                </p>
              </div>
              ...
              <Button
                ...
                disabled={loading || !plan || !legalAccepted}
              >
                {loading ? (
                  <>
