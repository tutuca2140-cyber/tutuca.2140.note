...
import { CreditCard, DollarSign, Percent, TrendingUp, Users, ArrowDownLeft, ArrowUpRight, Clock3, ReceiptText } from "lucide-react";
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3"><Card className="border-emerald-200"><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Lucro de veículos</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-emerald-600">{formatCurrency(stats?.vehicleProfit || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Vendas menos compra e despesas</p></CardContent></Card><Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Despesas de veículos</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-rose-600">{formatCurrency(stats?.vehicleExpenses || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Acumuladas no estoque ativo</p></CardContent></Card><Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Vendas de veículos</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-primary">{stats?.vehicleSalesCount || 0}</div><p className="mt-1 text-xs text-muted-foreground">Vendas registradas</p></CardContent></Card></div>

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5"><Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total emprestado</CardTitle><ArrowDownLeft className="h-4 w-4 text-rose-600" /></CardHeader><CardContent><div className="text-2xl font-bold">{formatCurrency(stats?.loanMetrics?.totalLent || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Contratos não cancelados</p></CardContent></Card><Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Juros em aberto</CardTitle><Percent className="h-4 w-4 text-primary" /></CardHeader><CardContent><div className="text-2xl font-bold text-primary">{formatCurrency(stats?.loanMetrics?.totalInterestOpen || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Incluídos no saldo devedor</p></CardContent></Card><Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total recebido</CardTitle><ArrowUpRight className="h-4 w-4 text-emerald-600" /></CardHeader><CardContent><div className="text-2xl font-bold text-emerald-600">{formatCurrency(stats?.loanMetrics?.totalReceived || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Pagamentos efetivos</p></CardContent></Card><Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Juros recebidos</CardTitle><ReceiptText className="h-4 w-4 text-primary" /></CardHeader><CardContent><div className="text-2xl font-bold">{formatCurrency(stats?.loanMetrics?.totalInterestReceived || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Juros alocados nos pagamentos</p></CardContent></Card><Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Amortização</CardTitle><CreditCard className="h-4 w-4 text-primary" /></CardHeader><CardContent><div className="text-2xl font-bold">{formatCurrency(stats?.loanMetrics?.totalPrincipalAmortized || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Principal amortizado</p></CardContent></Card></div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total em aberto</CardTitle><Clock3 className="h-4 w-4 text-primary" /></CardHeader><CardContent><div className="text-2xl font-bold text-primary">{formatCurrency(stats?.loanMetrics?.totalOpen || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Saldo principal e juros</p></CardContent></Card><Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Total vencido</CardTitle><Clock3 className="h-4 w-4 text-rose-600" /></CardHeader><CardContent><div className="text-2xl font-bold text-rose-600">{formatCurrency(stats?.loanMetrics?.totalOverdue || 0)}</div><p className="mt-1 text-xs text-muted-foreground">{stats?.loanMetrics?.overdueCount || 0} contrato(s)</p></CardContent></Card><Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Compras de veículos</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{formatCurrency(stats?.loanMetrics?.totalVehiclePurchases || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Saídas identificadas no caixa</p></CardContent></Card><Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Vendas de veículos</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-emerald-600">{formatCurrency(stats?.loanMetrics?.totalVehicleSales || 0)}</div><p className="mt-1 text-xs text-muted-foreground">Valor bruto das vendas</p></CardContent></Card></div>
