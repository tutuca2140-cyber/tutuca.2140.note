...
const ads = [
  ...
  {
    eyebrow: "Aluguéis",
    title: "Receba seus aluguéis com muito mais controle.",
    description: "Vincule imóvel e cliente, defina o vencimento e acompanhe o que está pago, pendente ou atrasado.",
    icon: KeyRound,
...
function RotatingAds() {
  ...
  useEffect(() => {
    const id = window.setInterval(() => setActive(index => (index + 1) % ads.length), 4500);
  ...
  return (
    <section aria-label="Destaques do Note Note" className="bg-white px-5 py-8 lg:px-8">
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-950 shadow-xl">
        <div className={`relative min-h-[300px] overflow-hidden bg-gradient-to-br ${ad.accent} px-7 py-9 text-white sm:px-10 lg:px-14 lg:py-12`}>
          <div className="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-white/10 blur-2xl" />
          <div className="relative grid items-center gap-8 lg:grid-cols-[1fr_.65fr]">
            <div>
            ...
            <div className="hidden justify-center lg:flex">
              <div className="flex h-44 w-44 items-center justify-center rounded-[2.5rem] border border-white/20 bg-white/10 shadow-2xl backdrop-blur-sm">
      ...
      <div className="relative overflow-hidden rounded-[1.75rem] border border-slate-200 bg-slate-950 p-2 shadow-[0_35px_90px_-35px_rgba(15,23,42,0.45)]">
        <div className="overflow-hidden rounded-[1.3rem] bg-white">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
            <div className="flex items-center gap-2"><img src="/brand/note-note-icon.png" alt="" className="h-7 w-7" /><span className="font-semibold text-slate-900">Note Note</span></div>
            <span className="rounded-full bg-blue-50 px-3 py-1 text-[11px] font-semibold text-blue-700">Demonstração do painel</span>
          </div>
          <div className="grid min-h-[390px] grid-cols-[130px_1fr] sm:grid-cols-[155px_1fr]">
            <aside className="border-r border-slate-100 bg-slate-50 p-3">
              <div className="mb-3 rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white">Painel</div>
              {["Clientes", "Empréstimos", "Veículos", "Produtos", "Imóveis", "Aluguéis", "Financiamentos", "Caixa", "Relatórios"].map(item => <div key={item} className="px-2 py-1.5 text-[10px] font-medium text-slate-500 sm:text-[11px]">{item}</div>)}
            </aside>
  ...
  return (
    <div className="min-h-screen bg-white text-slate-950">
      <div className="bg-slate-950 px-4 py-2.5 text-center text-xs font-extrabold uppercase tracking-[0.16em] text-white sm:text-sm">
        Fez um rolo? Deu negócio? <span className="text-sky-300">Controle seus recebimentos.</span>
      </div>
      ...
      <main>
        <section id="inicio" className="relative overflow-hidden bg-gradient-to-br from-white via-blue-50/80 to-sky-100/80">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 py-16 lg:grid-cols-[.9fr_1.1fr] lg:px-8 lg:py-24">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white/80 px-4 py-2 text-sm font-semibold text-blue-900 shadow-sm"><span className="text-blue-600">★</span>Feito para quem vende, empresta, aluga e negocia no dia a dia</div>
              <h1 className="max-w-3xl text-4xl font-black leading-[1.04] tracking-tight text-slate-950 sm:text-5xl lg:text-[64px]">Organize tudo o que movimenta o seu negócio <span className="text-blue-600">em um só lugar.</span></h1>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">O Note Note reúne clientes, empréstimos, veículos, produtos, imóveis, aluguéis, vendas financiadas, pagamentos, contas a receber, caixa e relatórios em uma única plataforma.</p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/planos"><a className="inline-flex h-14 items-center justify-center gap-2 rounded-xl bg-blue-600 px-7 text-base font-bold text-white shadow-xl shadow-blue-600/20 transition hover:-translate-y-0.5 hover:bg-blue-700">Ver planos <ArrowRight className="h-5 w-5" /></a></Link><a href="#recursos" className="inline-flex h-14 items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white px-7 text-base font-bold text-blue-700 shadow-sm transition hover:bg-blue-50"><PlayCircle className="h-5 w-5" />Ver como funciona</a></div>
              <div className="mt-9 grid gap-3 text-sm font-medium text-slate-600 sm:grid-cols-3"><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-blue-600" /> Acesso por usuário</div><div className="flex items-center gap-2"><Cloud className="h-5 w-5 text-blue-600" /> Acesse pela internet</div><div className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-blue-600" /> Simples de usar</div></div>
            </div>
            <DemoPanel />
          </div>
        </section>
        ...
        <section id="recursos" className="scroll-mt-24 bg-white py-20">
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
            <div className="mx-auto max-w-3xl text-center"><p className="text-sm font-bold uppercase tracking-[0.18em] text-blue-600">Seu negócio organizado</p><h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Tudo o que você precisa para acompanhar seus negócios</h2><p className="mt-4 text-base leading-7 text-slate-600">Uma visão simples do que você vendeu, emprestou, alugou, financiou, recebeu e ainda tem para receber.</p></div>
            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{features.map(feature => { const Icon = feature.icon; return <article key={feature.title} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-lg"><div className={`flex h-12 w-12 items-center justify-center rounded-xl ${feature.iconClass}`}><Icon className="h-6 w-6" /></div><h3 className="mt-4 text-base font-extrabold text-slate-900">{feature.title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{feature.description}</p></article>; })}</div>
          </div>
        </section>

        <section id="imoveis" className="scroll-mt-24 border-y border-blue-100 bg-gradient-to-br from-slate-50 to-blue-50/60 py-20">
          <div className="mx-auto max-w-7xl px-5 lg:px-8"><div className="grid items-center gap-10 lg:grid-cols-[.85fr_1.15fr]"><div><div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-600/20"><Building2 className="h-7 w-7" /></div><p className="mt-6 text-sm font-bold uppercase tracking-[0.18em] text-blue-600">Gestão de imóveis</p><h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Venda, financie e alugue imóveis dentro do Note Note.</h2><p className="mt-5 text-base leading-8 text-slate-600">Cadastre o patrimônio, acompanhe o cliente, registre contratos e mantenha os recebimentos ligados ao seu caixa.</p></div><div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><Building2 className="h-6 w-6 text-blue-600"/><h3 className="mt-4 font-extrabold text-slate-900">Cadastro completo</h3><p className="mt-2 text-sm leading-6 text-slate-500">Casa, apartamento, terreno ou loja, com endereço, tamanho, cômodos, quartos, salas, cozinha, banheiros, garagem, preço e observações.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><KeyRound className="h-6 w-6 text-emerald-600"/><h3 className="mt-4 font-extrabold text-slate-900">Controle de aluguéis</h3><p className="mt-2 text-sm leading-6 text-slate-500">Vincule imóvel e cliente, defina valor e dia de pagamento e acompanhe mensalmente o que foi pago, pendente ou atrasado.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><ClipboardList className="h-6 w-6 text-violet-600"/><h3 className="mt-4 font-extrabold text-slate-900">Venda e financiamento</h3><p className="mt-2 text-sm leading-6 text-slate-500">Venda à vista ou financie casas, apartamentos, terrenos e lojas, com entrada, juros, parcelas, saldo e acompanhamento do contrato.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><Wallet className="h-6 w-6 text-amber-600"/><h3 className="mt-4 font-extrabold text-slate-900">Tudo movimenta o caixa</h3><p className="mt-2 text-sm leading-6 text-slate-500">Recebimentos de aluguel, entradas, parcelas de financiamento e vendas de imóveis entram no controle financeiro da operação.</p></div></div></div></div>
        </section>
