...
import { Database, KeyRound, Pencil, Plus, Shield, UserRoundX } from "lucide-react";
...
type Draft = Permissions & { id?: number; username: string; email: string; name: string; password: string; role: "user" | "admin"; databaseIds: number[] };
  ...
  return <DashboardLayout><div className="space-y-6">
    <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center"><div><h1 className="text-3xl font-bold tracking-tight">Gerenciamento de Usuários</h1><p className="mt-2 text-muted-foreground">Crie contas e defina exatamente o que cada usuário pode fazer.</p></div>
      <Dialog open={open} onOpenChange={value => { setOpen(value); if (!value) setDraft(makeEmptyDraft()); }}><DialogTrigger asChild><Button className="w-full sm:w-auto"><Plus className="mr-2 h-4 w-4" />Novo usuário</Button></DialogTrigger><DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{draft.id ? "Editar usuário" : "Criar usuário"}</DialogTitle><DialogDescription>Configure os dados de acesso e as ações autorizadas para esta conta.</DialogDescription></DialogHeader><div className="space-y-6">
        ...
        <section className="space-y-4 border-t pt-5"><div className="flex items-start gap-3"><div className="rounded-lg bg-primary/10 p-2 text-primary"><Database className="h-5 w-5" /></div><div><h3 className="font-semibold">Bancos permitidos</h3><p className="text-sm text-muted-foreground">Vincule até três bancos em que este usuário poderá consultar e lançar informações.</p></div></div><div className="grid gap-3 sm:grid-cols-2">
          {databases.map(database => <label key={database.id} className="flex cursor-pointer items-center gap-3 rounded-xl border bg-card p-4"><Checkbox checked={draft.databaseIds.includes(database.id)} onCheckedChange={checked => toggleDatabase(database.id, checked === true)} /><span className="min-w-0"><span className="block truncate font-medium">{database.name}</span><span className="text-xs text-muted-foreground">{database.description || "Banco de dados operacional"}</span></span></label>)}
        ...
        </div><p className="text-xs font-medium text-primary">{draft.databaseIds.length}/3 bancos selecionados</p></section>
    ...
    {isLoading ? <p className="py-12 text-center text-muted-foreground">Carregando usuários...</p> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{users.map(user => { const protectedUser = user.username?.toLowerCase() === "draco"; return <Card key={user.id}><CardHeader><CardTitle className="flex items-center justify-between gap-2"><span className="truncate">{user.name || user.username || user.email}</span><Shield className="h-5 w-5 text-primary" /></CardTitle></CardHeader><CardContent className="space-y-4">
      <div><Badge>{user.role}</Badge><Badge variant="outline" className={`ml-2 ${user.isActive ? "text-green-700" : "text-red-700"}`}>{user.isActive ? "Ativo" : "Inativo"}</Badge></div><p className="text-sm text-muted-foreground">@{user.username}<br />{user.email}</p><div className="flex flex-wrap gap-1.5">{(user.databaseIds ?? []).map(databaseId => <Badge key={databaseId} variant="outline"><Database className="mr-1 h-3 w-3" />{databases.find(database => database.id === databaseId)?.name ?? `Banco #${databaseId}`}</Badge>)}</div><div className="flex flex-wrap gap-1.5">{permissionOptions.filter(permission => user[permission.key]).map(permission => <Badge key={permission.key} variant="secondary" className="font-normal">{permission.label}</Badge>)}</div>
