...
export default function AdminUsuarios() {
  ...
  const superAdminUsers = users.filter(user => !isCommercialAccount(user.loginMethod));
  const commercialUsers = users.filter(user => isCommercialAccount(user.loginMethod));
  const visibleUsers = activeTab === "commercial" ? commercialUsers : superAdminUsers;
  ...
  const toggle = async (userId: number, isActive: boolean) => {
    try {
      ...
      await refresh();
      toast.success(isActive ? "Usuário ativado." : "Usuário desativado e sessões encerradas.");
    } catch (error) {
      fail(error);
    }
  };
  ...
  const removeInternalUser = async (user: (typeof users)[number]) => {
    if (!window.confirm("Tem certeza que deseja realizar essa ação?")) return;
    ...
    try {
      const response = await fetch("/api/admin/control-panel", {
        ...
        credentials: "include",
        body: JSON.stringify({ action: "delete_internal_user", userId: user.id }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result?.success) {
        throw new Error(result?.message || "Não foi possível excluir o usuário.");
      }
      toast.success(result.message || "Usuário excluído do sistema.");
      await refresh();
    } catch (error) {
      fail(error);
    } finally {
  ...
  const toggleDatabase = (databaseId: number, checked: boolean) => {
    setDraft(current => {
      if (!checked) return { ...current, databaseIds: current.databaseIds.filter(id => id !== databaseId) };
      ...
      <div className="space-y-6">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">Contas gratuitas/teste do Super Admin e contas comerciais ficam separadas por origem.</p>
          </div>
          <Dialog
            open={open}
            onOpenChange={value => {
          ...
          >
            ...
            <DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{draft.id ? "Editar usuário" : "Criar usuário"}</DialogTitle>
                <DialogDescription>Configure os dados de acesso e as ações autorizadas para esta conta.</DialogDescription>
              </DialogHeader>
              <div className="space-y-6">
                <section className="space-y-4">
                  <div>
                    <h3 className="font-semibold">Dados de acesso</h3>
                    <p className="text-sm text-muted-foreground">Informe a identificação e o perfil principal da conta.</p>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      <Label htmlFor="user-name">Nome</Label>
                      <Input id="user-name" className="mt-2" value={draft.name} onChange={e => setDraft({ ...draft, name: e.target.value })} />
                    </div>
                    <div>
                    ...
                    {!draft.id && (
                      <div>
                        ...
                        <Input id="user-password" className="mt-2" type="password" minLength={6} value={draft.password} onChange={e => setDraft({ ...draft, password: e.target.value })} />
                    ...
                    <div className={draft.id ? "sm:col-span-2" : undefined}>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">Ao trocar o perfil, aplicamos as permissões recomendadas; você pode ajustá-las abaixo.</p>
                    </div>
                  </div>
                </section>
                ...
                <section className="space-y-4 border-t pt-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div><h3 className="font-semibold">Bancos permitidos</h3><p className="text-sm text-muted-foreground">Vincule até três bancos em que este usuário poderá consultar e lançar informações.</p></div>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    {databases.map(database => (
                      <label key={database.id} className="flex cursor-pointer items-center gap-3 rounded-xl border bg-card p-4">
                        <Checkbox checked={draft.databaseIds.includes(database.id)} onCheckedChange={checked => toggleDatabase(database.id, checked === true)} />
                        <span className="min-w-0"><span className="block truncate font-medium">{database.name}</span><span className="text-xs text-muted-foreground">{database.description || "Banco de dados operacional"}</span></span>
                      </label>
                    ))}
                    {!databases.length && <p className="text-sm text-muted-foreground sm:col-span-2">Crie um banco de dados antes de vinculá-lo ao usuário.</p>}
                  </div>
                  <p className="text-xs font-medium text-primary">{draft.databaseIds.length}/3 bancos selecionados</p>
                </section>

                <section className="space-y-4 border-t pt-5">
                  <div className="flex items-start gap-3">
                    <div className="rounded-lg bg-primary/10 p-2 text-primary"><KeyRound className="h-5 w-5" /></div>
                    <div><h3 className="font-semibold">Permissões concedidas</h3><p className="text-sm text-muted-foreground">Ative somente os recursos necessários para este usuário.</p></div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setDraft(current => ({ ...current, dashboardOnly: !current.dashboardOnly, canView: true, canInsert: false, canEdit: false, canDelete: false, canGenerateReports: false, canAccessSettings: false }))}
                  ...
                  >
                    <span className="block font-semibold">Somente Dashboard — opção mais simples</span>
                    <span className="mt-1 block text-sm text-muted-foreground">O usuário poderá visualizar apenas o dashboard completo do banco autorizado, sem acessar cadastros, caixa ou configurações.</span>
                  </button>
                  {!draft.dashboardOnly && (
                    <div className="grid gap-3 sm:grid-cols-2">
                      {permissionOptions.map(permission => (
                        <div key={permission.key} className="flex min-w-0 items-start justify-between gap-4 rounded-xl border bg-card p-4">
                          <Label htmlFor={`permission-${permission.key}`} className="min-w-0 cursor-pointer leading-normal">
                          ...
                          <Switch id={`permission-${permission.key}`} checked={draft[permission.key]} onCheckedChange={checked => setDraft(current => ({ ...current, [permission.key]: checked }))} aria-label={permission.label} />
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex flex-col-reverse gap-2 border-t pt-5 sm:flex-row sm:justify-end">
                  ...
                  <Button onClick={save} disabled={busy || !draft.name.trim() || !draft.username.trim() || !draft.email.trim() || (!draft.id && draft.password.length < 6)}>
                    {busy ? "Salvando..." : draft.id ? "Salvar alterações" : "Criar usuário"}
                  </Button>
                </div>
              </div>
            </DialogContent>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <button
            ...
            className={`rounded-2xl border p-5 text-left transition ${activeTab === "super_admin" ? "border-primary bg-primary/10 shadow-sm" : "bg-card hover:border-primary/40 hover:bg-muted/30"}`}
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                <div className="rounded-xl bg-primary/10 p-2.5 text-primary"><UserCog className="h-5 w-5" /></div>
                <div><p className="font-bold">Usuários Criados Pelo Super Admin</p><p className="mt-1 text-sm text-muted-foreground">Contas gratuitas, de teste e administrativas criadas diretamente pelo Super Admin.</p></div>
              </div>
              <Badge variant={activeTab === "super_admin" ? "default" : "secondary"}>{superAdminUsers.length}</Badge>
            </div>
          </button>
          ...
          <button
            ...
            className={`rounded-2xl border p-5 text-left transition ${activeTab === "commercial" ? "border-primary bg-primary/10 shadow-sm" : "bg-card hover:border-primary/40 hover:bg-muted/30"}`}
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                <div className="rounded-xl bg-primary/10 p-2.5 text-primary"><ShoppingBag className="h-5 w-5" /></div>
                <div><p className="font-bold">Usuários que Compraram Pelo Site</p><p className="mt-1 text-sm text-muted-foreground">Contratantes Basic/Plus e usuários adicionais vinculados às contas comerciais.</p></div>
              </div>
              <Badge variant={activeTab === "commercial" ? "default" : "secondary"}>{commercialUsers.length}</Badge>
            </div>
          </button>
        </div>
        ...
        {activeTab === "commercial" && (
          <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950">
            <strong>Contas comerciais:</strong> contratantes são identificados pelo cadastro feito no site. Para esses clientes, a recuperação da própria senha é feita pelo e-mail cadastrado.
          </div>
        )}

        {isLoading ? (
          <p className="py-12 text-center text-muted-foreground">Carregando usuários...</p>
        ) : visibleUsers.length ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {visibleUsers.map(user => {
              const protectedUser = user.username?.toLowerCase() === "draco" || user.role === "super_admin";
                ...
                <Card key={user.id}>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex flex-wrap gap-2">
                      <Badge>{user.dashboardOnly ? "Somente Dashboard" : user.role}</Badge>
                      <Badge variant="outline" className={user.isActive ? "text-green-700" : "text-red-700"}>{user.isActive ? "Ativo" : "Inativo"}</Badge>
                      {commercialOwner && <Badge variant="secondary">Contratante do site</Badge>}
                      {commercialSubuser && <Badge variant="secondary">Usuário da conta</Badge>}
                      {!isCommercialAccount(user.loginMethod) && <Badge variant="secondary">Criado pelo Super Admin</Badge>}
                    </div>
                    <p className="text-sm text-muted-foreground">@{user.username}<br />{user.email}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {(user.databaseIds ?? []).map(databaseId => (
                        <Badge key={databaseId} variant="outline"><Database className="mr-1 h-3 w-3" />{databases.find(database => database.id === databaseId)?.name ?? `Banco #${databaseId}`}</Badge>
                      ))}
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {permissionOptions.filter(permission => user[permission.key]).map(permission => (
                        <Badge key={permission.key} variant="secondary" className="font-normal">{permission.label}</Badge>
                      ))}
                    </div>

                    {protectedUser ? (
                      <p className="rounded-md bg-primary/5 p-3 text-xs text-primary">Super administrador protegido.</p>
                    ) : (
                      <>
                        <div className="flex flex-wrap gap-2">
                          ...
                          {activeTab === "super_admin" && !isCommercialAccount(user.loginMethod) && (
                            <Button variant="destructive" size="sm" onClick={() => removeInternalUser(user)} disabled={deletingId === user.id}>
                              <Trash2 className="mr-2 h-4 w-4" />{deletingId === user.id ? "Excluindo..." : "Excluir usuário"}
                            </Button>
                          )}
                        </div>

                        {commercialOwner ? (
                          <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-xs leading-5 text-blue-950">
                            O contratante recupera a senha pelo e-mail cadastrado usando “Esqueci minha senha” na tela de login.
                          </div>
                        ) : (
                          <div className="flex flex-col gap-2 sm:flex-row">
                            ...
                            />
                            <Button className="w-full sm:w-auto" variant="outline" onClick={() => reset(user.id)}>Redefinir</Button>
                          </div>
                        )}
                      </>
                    )}
          ...
          <Card>
            <CardContent className="py-12 text-center text-muted-foreground">
              {activeTab === "commercial" ? "Nenhum usuário comercial cadastrado até o momento." : "Nenhum usuário criado pelo Super Admin encontrado."}
            </CardContent>
          </Card>
        )}
      </div>
