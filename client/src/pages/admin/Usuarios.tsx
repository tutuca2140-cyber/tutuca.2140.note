...
export default function AdminUsuarios() {
  ...
  return <DashboardLayout><div className="space-y-6">
    <div className="flex items-start justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight">Gerenciamento de Usuários</h1><p className="mt-2 text-muted-foreground">Crie, edite, desative e redefina senhas.</p></div>
      <Dialog open={open} onOpenChange={value => { setOpen(value); if (!value) setDraft(emptyDraft); }}><DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" />Novo usuário</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>{draft.id ? "Editar usuário" : "Criar usuário"}</DialogTitle></DialogHeader><div className="grid gap-4">
    ...
    {isLoading ? <p className="py-12 text-center text-muted-foreground">Carregando usuários...</p> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{users.map(user => { const protectedUser = user.username?.toLowerCase() === "draco"; return <Card key={user.id}><CardHeader><CardTitle className="flex items-center justify-between gap-2"><span className="truncate">{user.name || user.username || user.email}</span><Shield className="h-5 w-5 text-primary" /></CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      {protectedUser ? <p className="rounded-md bg-blue-50 p-2 text-xs text-blue-700">Super administrador protegido.</p> : <><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => edit(user)}><Pencil className="mr-2 h-4 w-4" />Editar</Button><Button variant="outline" size="sm" onClick={() => toggle(user.id, !user.isActive)}><UserRoundX className="mr-2 h-4 w-4" />{user.isActive ? "Desativar" : "Ativar"}</Button></div><div className="flex gap-2"><Input type="password" placeholder="Nova senha" value={passwordDrafts[user.id] ?? ""} onChange={e => setPasswordDrafts(current => ({ ...current, [user.id]: e.target.value }))} /><Button variant="outline" onClick={() => reset(user.id)}>Redefinir</Button></div></>}
