...
import { CheckSquare, Image, Mail, Send, Square, Users } from "lucide-react";
...
export default function Marketing() {
  ...
  const send=async()=>{
    ...
    setSending(true);
    try { const r=await fetch("/api/admin/marketing",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({segment,selectedIds:selected,subject,message,imageUrl})}); const j=await r.json(); if(!r.ok) throw new Error(j.message); toast.success(`Campanha concluída: ${j.sent} enviados${j.failed?`, ${j.failed} falharam`:""}.`); }
  ...
  return <DashboardLayout><div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_1.05fr]">
      ...
      <section className="rounded-2xl border bg-background p-5 space-y-4"><div><h2 className="font-semibold">Criar campanha</h2><p className="text-xs text-muted-foreground">O e-mail recebe automaticamente a identidade visual do Note Note.</p></div>
        ...
        <div><label className="mb-1 block text-sm font-medium">Imagem da campanha</label><div className="flex gap-2"><Image className="mt-2.5 h-5 w-5 text-muted-foreground"/><Input value={imageUrl} onChange={e=>setImageUrl(e.target.value)} placeholder="https://... endereço público da imagem"/></div><p className="mt-1 text-xs text-muted-foreground">Cole uma URL HTTPS de uma imagem. Ela aparecerá em destaque no topo do conteúdo.</p></div>
        {imageUrl&&/^https:\/\//i.test(imageUrl)&&<div className="overflow-hidden rounded-xl border"><img src={imageUrl} alt="Prévia da campanha" className="max-h-64 w-full object-cover"/></div>}
