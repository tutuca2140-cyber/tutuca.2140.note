...
import { CheckSquare, Mail, Scissors, Send, Square, Trash2, Upload, Users } from "lucide-react";

type Recipient = { id:number; supportId?:string|null; name?:string; username?:string; email:string; whatsapp?:string; plan?:string; status?:string; billingMethod?:string; barbershopName?:string; barbershopSlug?:string; marketingState:"current"|"overdue"|"other" };

export default function Marketing() {
  const [recipients,setRecipients]=useState<Recipient[]>([]);
  const [loading,setLoading]=useState(true);
  const [sending,setSending]=useState(false);
  const [segment,setSegment]=useState<"all"|"current"|"overdue"|"barber"|"selected">("all");
  ...
  const visible=useMemo(()=>segment==="current"?recipients.filter(r=>r.marketingState==="current"):segment==="overdue"?recipients.filter(r=>r.marketingState==="overdue"):segment==="barber"?recipients.filter(r=>r.plan==="barber"):recipients,[recipients,segment]);
  ...
  return <DashboardLayout><div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <button onClick={()=>setSegment("barber")} className={`rounded-2xl border p-4 text-left ${segment==="barber"?"border-primary bg-primary/5":"bg-background"}`}><Scissors className="mb-2 h-5 w-5"/><b>Barbearias</b><div className="text-2xl font-bold">{recipients.filter(r=>r.plan==="barber").length}</div></button>
    </div>
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_1.05fr]">
      ...
      <section className="rounded-2xl border bg-background p-5 space-y-4"><div><h2 className="font-semibold">Criar campanha</h2><p className="text-xs text-muted-foreground">O e-mail recebe automaticamente a identidade visual do Note Note.</p></div>
        ...
        <div className="rounded-xl bg-muted/50 p-3 text-sm"><b>Envio atual:</b> {segment==="all"?"todos os clientes":segment==="current"?"clientes com pagamento em dia":segment==="overdue"?"clientes com pagamento atrasado":segment==="barber"?"clientes do plano Barbearia":`${selected.length} clientes selecionados`}.</div>
