...
function paymentBadge(account: CommercialAccount) {
  if (account.paymentState === "trial")
    return <Badge className="bg-blue-600">Teste grátis</Badge>;
  if (account.paymentState === "paid")
    return <Badge className="bg-emerald-600">Ativo / pago</Badge>;
  if (account.paymentState === "unpaid")
    return <Badge variant="destructive">Não pago</Badge>;
  if (account.paymentState === "canceled")
...
export default function AdminAssinaturas() {
  ...
  const [secureTarget, setSecureTarget] = useState<CommercialAccount | null>(
    null
  );
  ...
  useEffect(() => {
    if (
      !authLoading &&
      user &&
      !(user.role === "super_admin" || user.adminCanSubscriptions)
    )
  ...
  useEffect(() => {
    if (user?.role === "super_admin" || user?.adminCanSubscriptions)
      void load();
  }, [user?.role]);

  const runAction = async (
    account: CommercialAccount,
    action: "approve" | "mark_unpaid" | "mark_paid"
  ) => {
    ...
    try {
      ...
      if (!response.ok || !result?.success)
        throw new Error(
          result?.message || "Não foi possível atualizar a assinatura."
        );
  ...
  const confirmSecureAction = async () => {
    ...
    if (!adminPassword)
      return toast.error("Digite a senha do Super Administrador.");
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Visão comercial completa: plano, cobrança, pagamento, teste,
              validade, Asaas e acesso do cliente.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              onClick={() => downloadAccountsExcel(data?.accounts ?? [])}
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950">
          <strong>Relatório comercial:</strong> o Excel inclui contato,
          WhatsApp, e-mail, plano, pagamento, teste, cancelamento, inatividade,
          validade, dados do Asaas, acessos e bancos vinculados.
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-7">
          ...
          <Card>
            <CardContent className="p-5">
              <p className="text-xs text-muted-foreground">Cartão / mês</p>
              <p className="mt-1 text-xl font-black">
                {money.format(
                  Number(data?.summary.monthlyActiveCents || 0) / 100
                )}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-5">
              <p className="text-xs text-muted-foreground">Pix anual ativo</p>
              <p className="mt-1 text-xl font-black">
                {money.format(
                  Number(data?.summary.annualPixActiveCents || 0) / 100
                )}
              </p>
            </CardContent>
          </Card>
            ...
            {loading && !data ? (
              ...
              <div className="space-y-4">
                {(data?.accounts ?? []).map(account => {
                  ...
                  const active =
                    account.status === "active" || account.status === "paid";
                  return (
                    <div
                      key={account.id}
                    ...
                    >
                      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                        ...
                        <div className="flex flex-wrap gap-2 xl:justify-end">
                          {account.status === "pending_payment" && (
                            <Button
                              size="sm"
                              onClick={() => runAction(account, "approve")}
                          ...
                          {account.status === "past_due" && (
                            <Button
                              size="sm"
                              onClick={() => runAction(account, "mark_paid")}
                              disabled={actionId === account.id}
                            >
                              <CheckCircle2 className="mr-2 h-4 w-4" />
                              Confirmar pagamento
                          ...
                          {active && (
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => runAction(account, "mark_unpaid")}
                              disabled={actionId === account.id}
                            >
                          ...
                          {active && account.billingMethod === "pix_annual" && (
                            <Button
                              size="sm"
                              variant="outline"
                              disabled
                              title="Fluxo de estorno será habilitado em uma próxima etapa"
                            >
                          ...
                          {account.status !== "canceled" && (
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                openSecureAction(account, "cancel_subscription")
                              }
                              disabled={actionId === account.id}
                            >
                          ...
                          {canDelete && (
                            <Button
                              ...
                              variant="destructive"
                              onClick={() =>
                                openSecureAction(account, "delete_unpaid")
                              }
                              disabled={actionId === account.id}
                            >
                      ...
                      <div className="mt-4 grid gap-3 md:grid-cols-3">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          ...
                          <span>
                            Uso:{" "}
                            <strong>{formatUsage(account.usageMinutes)}</strong>
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {!data?.accounts?.length && (
                  <div className="py-12 text-center text-muted-foreground">
                    Nenhuma assinatura comercial encontrada.
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 font-black">
                  ...
                  Confirmação do Super Admin
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  {secureAction === "delete_unpaid"
                    ? `Excluir definitivamente a conta de ${secureTarget.name || secureTarget.username}?`
                    : `Cancelar a assinatura de ${secureTarget.name || secureTarget.username}?`}
                </p>
              </div>
              <button onClick={() => !secureLoading && setSecureTarget(null)}>
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
              {secureAction === "delete_unpaid"
                ? "A cobrança/assinatura pendente no Asaas será removida antes dos dados locais sempre que houver um recurso vinculado."
                : "O sistema tentará encerrar a recorrência no Asaas e bloqueará o acesso do cliente."}
            </div>
