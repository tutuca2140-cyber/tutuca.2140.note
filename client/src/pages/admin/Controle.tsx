...
type ControlData = {
  ...
  onlineUsers: Array<Record<string, any>>;
...
function lifecycleLabel(account: any) {
  ...
  const trialEndsAt = account.trialEndsAt
    ...
    : 0;
  if (account.status === "canceled" || account.paymentState === "canceled")
    return "Contrato cancelado";
  if (account.billingMethod === "pix_annual" && paidUntil && paidUntil < now)
    return "Contrato concluído / vencido";
  if (!account.isActive && account.status !== "pending_payment")
    return "Cliente inativo";
  if (account.status === "past_due") return "Inadimplente / pagamento pendente";
  if (account.status === "pending_payment")
    return "Pré-cadastro / não concluiu contratação";
  if (trialEndsAt > now && account.paymentState === "trial")
    return "Período de teste";
  if (account.status === "active" || account.status === "paid")
...
function exportCommercialExcel(accounts: any[]) {
  if (!accounts.length)
    return toast.error("Não há clientes comerciais para exportar.");
  const headers = [
    "ID interno",
    "ID de usuário",
    "Nome completo",
  ...
  const rows = accounts.map(account => [
    ...
    account.billingMethod === "pix_annual"
      ? safeDate(account.paidUntil, false)
      : "Renovação mensal",
...
export default function AdminControle() {
  ...
  const [filter, setFilter] = useState<
    "all" | "active" | "commercial" | "internal"
  >("all");
  ...
  useEffect(() => {
    if (
      !authLoading &&
      user &&
      !(user.role === "super_admin" || user.adminCanControlPanel)
    )
  ...
  useEffect(() => {
    if (!(user?.role === "super_admin" || user?.adminCanControlPanel)) return;
    void load();
    const timer = window.setInterval(() => void load(true), 30_000);
    return () => window.clearInterval(timer);
  }, [user?.role, user?.adminCanControlPanel]);
  ...
  const downloadDetailedReport = async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.success)
        throw new Error(
          result?.message || "Não foi possível gerar o relatório."
        );
  ...
  const storagePercent = Math.min(
    100,
    Math.max(0, Number(storage.usedPercent || 0))
  );
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-muted-foreground">
              Visão central dos acessos ao Note Note, usuários, assinaturas,
              valores, bancos e movimentação operacional.
            </p>
          </div>
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950">
          <strong>Excel comercial completo:</strong> o mesmo relatório detalhado
          da área de Assinaturas pode ser baixado daqui, incluindo status,
          pagamento, cancelamento, inatividade, contatos, WhatsApp, e-mail,
          plano, Asaas, acessos e bancos.
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            note={`${Number(s?.users?.total || 0)} usuários cadastrados`}
            icon={UserCheck}
          />
          <StatCard
            title="Clientes comerciais"
            value={Number(s?.users?.commercial || 0)}
            note={`${Number(s?.subscriptions?.pending || 0)} aguardando pagamento`}
            icon={Users}
          />
          <StatCard
            title="Acessos hoje"
            value={Number(s?.accesses?.today || 0)}
            note={`${Number(s?.accesses?.seven_days || 0)} nos últimos 7 dias`}
            icon={Activity}
          />
          <StatCard
            ...
            {data?.onlineUsers?.length ? (
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {data.onlineUsers.map(item => (
                  <div
                    key={item.id}
                    className="flex items-center gap-3 rounded-xl border bg-emerald-500/[0.04] p-4"
                  >
                    <div className="relative grid h-11 w-11 shrink-0 place-items-center rounded-full bg-primary/10 font-bold text-primary">
                      {String(item.name || item.username || "U")
                        .charAt(0)
                        .toUpperCase()}
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-mono text-xs text-primary">
                        ID: {item.supportId || item.id}
                      </p>
                      <p className="truncate text-xs text-muted-foreground">
                        Ativo em {item.path || "no sistema"} ·{" "}
                        {safeDate(item.lastActivityAt)}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="rounded-xl border border-dashed py-8 text-center text-sm text-muted-foreground">
                Nenhum usuário ativo neste momento.
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            value={cents(s?.subscriptions?.pendingValueCents)}
            note="Cadastros comerciais ainda sem confirmação de pagamento"
            icon={Banknote}
          />
          <StatCard
          ...
          <StatCard
            ...
            value={Number(s?.databases?.total || 0)}
            note={`${Number(s?.databases?.active || 0)} marcados como ativos`}
            icon={Database}
          />
        </div>
          ...
          <CardHeader>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Uso do armazenamento PostgreSQL comparado ao limite atual do
                  plano Neon.
                </p>
              </div>
              <Badge variant={storageWarning ? "destructive" : "secondary"}>
                Plano {storage.plan || "Free"} ·{" "}
          ...
          <CardContent className="space-y-5">
            <div>
              <div className="mb-2 flex items-end justify-between gap-4">
                ...
                <p
                  className={`text-lg font-bold ${storageWarning ? "text-amber-600" : "text-primary"}`}
                >
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
              <div className="rounded-xl border p-4">
                ...
                <p className="mt-1 text-xl font-bold">
                  {Number(s?.operations?.clients || 0)}
                </p>
              </div>
              <div className="rounded-xl border p-4">
                <p className="text-xs text-muted-foreground">
                  Empréstimos ativos/atrasados
                </p>
                <p className="mt-1 text-xl font-bold">
                  {Number(s?.operations?.active_loans || 0)}
                </p>
              </div>
              <div className="rounded-xl border p-4">
                <p className="text-xs text-muted-foreground">
                ...
                <p className="mt-1 text-xl font-bold">
                  {amount(s?.operations?.entries)}
                </p>
              </div>
              <div className="rounded-xl border p-4">
                <p className="text-xs text-muted-foreground">Saídas lançadas</p>
                <p className="mt-1 text-xl font-bold">
                  {amount(s?.operations?.exits)}
                </p>
              </div>
              <div className="rounded-xl border p-4">
                <p className="text-xs text-muted-foreground">Saldo agregado</p>
                <p className="mt-1 text-xl font-bold">
                  {amount(s?.operations?.balance)}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <div className="grid gap-4 lg:grid-cols-4">
          {[
            ...
            [
              "Gerenciar bancos",
              "Criar, editar, duplicar e administrar os bancos de dados.",
              "/admin/bancos",
              Database,
            ],
            [
              "Auditoria",
              "Consultar alterações e ações administrativas registradas.",
              "/admin/auditoria",
              History,
            ],
            [
              "Configurações",
              "Ajustar configurações administrativas da plataforma.",
              "/admin/configuracoes",
              Building2,
            ],
          ...
          <CardHeader>
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Lista consolidada para facilitar a administração dos acessos.
                </p>
              </div>
              <div className="flex flex-col gap-2 sm:flex-row">
                <div className="relative min-w-[260px]">
                  ...
                  <Input
                    ...
                    placeholder="Buscar nome, ID, usuário, e-mail ou banco"
                    className="pl-9"
                  />
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  {filteredUsers.map(item => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-muted/20">
                        ...
                        <td className="px-4 py-3">
                          <Badge variant={commercial ? "default" : "secondary"}>
                            {commercial ? "Comercial" : "Grátis/Teste"}
                          </Badge>
                        </td>
                        <td className="px-4 py-3">
                          {item.plan ? (
                        ...
                        <td className="px-4 py-3">
                          <Badge
                            variant={
                              item.hasActiveSession ? "default" : "secondary"
                            }
                          >
        ...
        <div className="grid gap-4 xl:grid-cols-[1.35fr_.65fr]">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Últimos 100 acessos registrados.
              </p>
            </CardHeader>
            <CardContent>
                  ...
                  <tbody className="divide-y">
                    {(data?.recentAccess ?? []).map(item => (
                      <tr key={item.id}>
                        ...
                        <td className="px-4 py-3 font-medium">{item.path}</td>
                        <td className="px-4 py-3">
                    ...
                    {!data?.recentAccess?.length && (
                      <tr>
                        ...
                        >
                          Os primeiros acessos aparecerão aqui.
                        </td>
                      </tr>
                    )}
                  </tbody>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {item.username || "Sistema"} · {item.entity || "—"}
                      </p>
                    </div>
                    <Badge
                      variant={
              ...
              {!data?.recentAudits?.length && (
                <p className="text-sm text-muted-foreground">
                  Nenhuma atividade de auditoria disponível.
                </p>
              )}
            </CardContent>
          </Card>
        ...
        {data?.generatedAt ? (
          <p className="text-right text-xs text-muted-foreground">
            Atualizado em {safeDate(data.generatedAt)}
          </p>
        ) : null}
      </div>
    </DashboardLayout>
