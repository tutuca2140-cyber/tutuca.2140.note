...
export default function Perfil() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.success) {
        throw new Error(
          result?.message || "Não foi possível carregar seu perfil."
        );
      }
  ...
  useEffect(() => {
    ...
    fetch("/api/site-access?scope=reviews&action=mine", {
      credentials: "include",
      cache: "no-store",
    })
      .then(async response => ({
  ...
  const saveReview = async () => {
    ...
    if (reviewComment.trim().length < 3)
      return toast.error("Escreva um comentário, sugestão ou elogio.");
    setReviewSaving(true);
    try {
      const response = await fetch(
        "/api/site-access?scope=reviews&action=mine",
        {
          method: "POST",
          credentials: "include",
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.success) {
        throw new Error(
          result?.message || "Não foi possível atualizar seu perfil."
        );
      }
  ...
  const cancelPlan = async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.success) {
        throw new Error(
          result?.message || "Não foi possível cancelar o plano."
        );
      }
  ...
  const deleteAccount = async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.success) {
        throw new Error(
          result?.message || "Não foi possível excluir sua conta."
        );
      }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-muted-foreground">
            Consulte seus dados, acompanhe o plano contratado, o teste grátis e
          ...
          <>
                  ...
                  <div className="min-w-0">
                    ...
                    {profile.priceCents != null && (
                      <p className="text-xs text-muted-foreground">
                        {money.format(Number(profile.priceCents) / 100)} ·{" "}
                        {billingLabel(profile)}
                      </p>
                    )}
                  </div>
                </CardContent>
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <CardDescription className="mt-1">
                        Veja exatamente o período de teste, forma de pagamento,
                        situação e validade do seu plano.
                      </CardDescription>
                    </div>
                    <Badge
                      variant={
                ...
                <CardContent className="space-y-4">
                  {profile.trialActive ? (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm">
                            {trialDays === 1
                              ? "Falta 1 dia"
                              : `Faltam ${trialDays} dias`}{" "}
                            para o fim do teste. Ele termina em{" "}
                            <strong>{safeDate(profile.trialEndsAt)}</strong>.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="rounded-xl border bg-muted/30 p-4 text-sm">
                      O período de teste grátis terminou em{" "}
                      <strong>{safeDate(profile.trialEndsAt)}</strong>.
                    </div>
                  ) : null}
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div className="rounded-xl border p-4">
                      ...
                      <p className="mt-1 font-bold">
                        {safeDate(profile.trialEndsAt)}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {profile.trialActive
                          ? `${trialDays} dia(s) restante(s)`
                          : "Encerrado ou não aplicável"}
                      </p>
                    </div>
                    <div className="rounded-xl border p-4">
                      <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                      ...
                      <p className="mt-1 font-bold">
                        {profile.billingMethod === "pix_annual"
                          ? safeDate(profile.paidUntil)
                          : "Mensal"}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {profile.billingMethod === "pix_annual"
                  ...
                  {canceledSubscription ? (
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-800">
                      <strong>Plano cancelado:</strong> o cancelamento já foi
                      registrado. Você ainda pode excluir definitivamente sua
                      conta pela seção abaixo.
                    </div>
                  ) : profile.canCancelPlan ? (
                    <div className="flex flex-col gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="font-semibold text-amber-950">
                          Deseja interromper o plano?
                        </p>
                        <p className="text-sm text-amber-900">
                          O cancelamento interrompe novas cobranças recorrentes
                          e altera a situação da assinatura para cancelada.
                        </p>
                      </div>
                      <Button
                        variant="outline"
            ...
            {!profile.editable && (
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-100">
                <strong>Perfil administrado:</strong> esta conta não foi criada
                pelo cadastro comercial principal do site. Alterações sensíveis
                continuam sob responsabilidade de quem administra a conta.
              </div>
            )}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Os mesmos dados informados na criação da conta ficam salvos
                  aqui.
                </CardDescription>
              </CardHeader>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Usado para entrar no Note Note.
                    </p>
                  </div>
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-muted-foreground">
                      Usado também para recuperação de senha.
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-muted-foreground">
                      Número gerado pelo sistema e não pode ser alterado.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="rounded-xl border bg-muted/30 p-4">
                    <div className="mb-4 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Para trocar usuário, e-mail ou senha, confirme
                          primeiro sua senha atual.
                        </p>
                      </div>
                    </div>
                    <div className="grid gap-4 md:grid-cols-3">
            ...
            {profile.commercial && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Sua opinião ajuda a melhorar o sistema. Dê até 5 estrelas e
                    escreva uma sugestão ou elogio. O texto só aparece no site
                    se for aprovado pelo Note Note.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <div className="flex gap-1.5">
                      {[1, 2, 3, 4, 5].map(n => (
                        ...
                        >
                          <Star
                            className={`h-8 w-8 ${n <= (reviewHover || reviewRating) ? "fill-amber-400 text-amber-400" : "text-muted-foreground/30"}`}
                          />
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <Label htmlFor="profile-review">
                      Comentário, sugestão ou elogio
                    </Label>
                    <textarea
                      id="profile-review"
                      value={reviewComment}
                      ...
                      maxLength={1200}
                      rows={5}
                      placeholder="Conte o que você acha do Note Note ou deixe uma sugestão..."
                      className="mt-2 w-full resize-y rounded-xl border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30"
                    />
                    <div className="mt-1 flex items-center justify-between gap-2 text-xs text-muted-foreground">
                      <span>
                        {reviewPublished
                          ? "Este depoimento está publicado no site."
                          : "Ao editar e reenviar, a avaliação volta para análise antes de ser publicada."}
                      </span>
                ...
                <CardHeader>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <CardDescription className="mt-1">
                        Antes da exclusão, o Note Note cancela a
                        assinatura/cobrança vinculada no Asaas quando houver um
                        recurso ativo. Depois disso, remove o acesso comercial,
                        os bancos pertencentes à conta e os dados operacionais.
                        No Plus, usuários adicionais vinculados também perdem o
                        acesso.
                      </CardDescription>
                    </div>
                  </div>
            ...
            <Dialog
              open={cancelOpen}
              onOpenChange={open => !cancelingPlan && setCancelOpen(open)}
            >
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Cancelar seu plano?</DialogTitle>
                  <DialogDescription>
                    Confirme sua senha para cancelar a assinatura. Esta ação não
                    exclui sua conta nem seus dados; a exclusão continua sendo
                    uma ação separada.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-2">
                  {profile.trialActive ? (
                    <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950">
                      ...
                      <strong>{safeDate(profile.trialEndsAt)}</strong> e restam{" "}
                      <strong>{trialDays} dia(s)</strong>.
                    </div>
                  ) : null}
                  <div>
                  ...
                  <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
                    ...
                    <p>
                      O Note Note tentará cancelar primeiro a recorrência ou
                      cobrança pendente vinculada ao Asaas e só depois
                      registrará o plano como cancelado.
                    </p>
                  </div>
                </div>
                <DialogFooter>
                  ...
                  <Button
                    variant="destructive"
                    onClick={cancelPlan}
                    disabled={cancelingPlan || !cancelPassword}
                  >
                    {cancelingPlan ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : (
            ...
            <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Esta ação não pode ser desfeita. O plano será cancelado
                    automaticamente antes da exclusão. Confirme sua senha e
                    digite EXCLUIR CONTA para continuar.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-2">
                  <div>
                  ...
                  <div className="flex items-start gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm">
                    ...
                    <p>
                      O sistema cancela o plano vinculado e exige duas
                      confirmações para impedir exclusões acidentais.
                    </p>
                  </div>
                </div>
                <DialogFooter>
