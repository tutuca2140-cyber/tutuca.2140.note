...
export default function OliviaFloatingAssistant() {
  ...
  async function sendContent(rawContent: string) {
    ...
    try {
      ...
      setMessages(current => [
        ...
        {
          role: "assistant",
          content: error instanceof Error ? error.message : "Não consegui concluir essa resposta agora.",
        },
      ]);
    }
  }
  ...
  return (
    <>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <header className="relative flex items-center gap-3 overflow-hidden bg-gradient-to-r from-slate-950 via-blue-950 to-cyan-900 px-4 py-3.5 text-white">
              ...
              <div className="relative">
                ...
                <img src={OLIVIA_AVATAR} alt="Olivia" className="relative h-12 w-12 rounded-full border-2 border-cyan-300 object-cover shadow-[0_0_18px_rgba(103,232,249,0.7)]" />
              </div>
              <div className="relative min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <p className="bg-gradient-to-r from-white via-cyan-100 to-cyan-300 bg-clip-text text-lg font-bold tracking-wide text-transparent">Olivia</p>
              ...
              <Button type="button" variant="ghost" size="icon" onClick={() => setOpen(false)} className="relative text-white hover:bg-white/10 hover:text-white" aria-label="Fechar Olivia">
                <X className="h-5 w-5" />
              </Button>
            </header>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto bg-[linear-gradient(135deg,rgba(8,47,73,0.05)_25%,transparent_25%,transparent_50%,rgba(8,47,73,0.05)_50%,rgba(8,47,73,0.05)_75%,transparent_75%)] bg-[length:26px_26px] p-4">
              {messages.map((item, index) => (
                <div key={index} className={`flex items-end gap-2 ${item.role === "user" ? "justify-end" : "justify-start"}`}>
                  {item.role === "assistant" && <img src={OLIVIA_AVATAR} alt="" className="h-7 w-7 rounded-full border border-cyan-300 object-cover shadow-sm" />}
                  <div className={`max-w-[84%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed shadow-sm ${item.role === "user" ? "rounded-br-md bg-gradient-to-br from-blue-600 to-cyan-600 text-white" : "rounded-bl-md border border-cyan-500/15 bg-card/95 text-card-foreground shadow-[0_8px_24px_rgba(8,145,178,0.08)]"}`}>
              ...
              {isPending && messages[messages.length - 1]?.content !== "" && (
                <div className="flex items-end gap-2">
                  <img src={OLIVIA_AVATAR} alt="" className="h-7 w-7 rounded-full border border-cyan-300 object-cover" />
            ...
            <form onSubmit={send} className="border-t bg-background/95 p-3">
              <div className="flex items-center gap-1 rounded-full border bg-muted/40 p-1.5 pl-4 focus-within:ring-2 focus-within:ring-cyan-500/40">
                <Input value={message} onChange={event => setMessage(event.target.value)} placeholder={voice.listening ? "Ouvindo sua pergunta..." : "Pergunte à Olivia..."} className="h-9 border-0 bg-transparent px-0 shadow-none focus-visible:ring-0" maxLength={500} disabled={isPending} />
                {memory.voiceEnabled && voice.listeningSupported && (
                  <Button type="button" size="icon" variant="ghost" className="h-9 w-9 shrink-0 rounded-full" onClick={voice.listening ? voice.stopListening : voice.startListening} aria-label={voice.listening ? "Parar de ouvir" : "Perguntar por voz"} title="Perguntar por voz">
                    {voice.listening ? <MicOff className="h-4 w-4 text-rose-500" /> : <Mic className="h-4 w-4 text-cyan-600" />}
                  </Button>
                )}
                <Button type="submit" size="icon" className="h-9 w-9 shrink-0 rounded-full bg-gradient-to-br from-blue-600 to-cyan-500" disabled={!message.trim() || isPending} aria-label="Enviar mensagem">
      ...
      {showInvitation && !open && (
        <button type="button" onClick={() => { setShowInvitation(false); setOpen(true); }} className={`fixed z-[69] animate-in fade-in slide-in-from-bottom-2 rounded-2xl border border-cyan-300/40 bg-slate-950 px-4 py-2.5 text-sm font-medium text-white shadow-xl ${position.x > viewport.width / 2 ? "-translate-x-full" : ""}`} style={{ left: position.x > viewport.width / 2 ? position.x - 10 : position.x + iconSize + 10, top: position.y + 12 }}>
      ...
      >
        ...
        <img src={OLIVIA_AVATAR} alt="" draggable={false} className="h-full w-full select-none rounded-full object-cover" />
