...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navigation = [
    ...
    { name: "Empréstimos", href: "/emprestimos", icon: CreditCard, show: user?.canView && regularAccess },
    { name: "Pagamentos", href: "/pagamentos", icon: Wallet, show: user?.canView && regularAccess },
    ...
    { name: "Produtos", href: "/produtos", icon: Package, show: user?.canView && regularAccess },
    { name: "Financiamentos", href: "/financiamentos", icon: ClipboardList, show: user?.canView && regularAccess },
    { name: "Contas a receber", href: "/contas-a-receber", icon: CalendarDays, show: user?.canView && regularAccess },
    { name: "Relatórios", href: "/relatorios", icon: FileText, show: user?.canGenerateReports && regularAccess },
  ];

  const adminNavigation = [
    { name: "Usuários", href: "/admin/usuarios", icon: Shield, show: isSuperAdmin },
    { name: "Sugestões", href: "/admin/sugestoes", icon: MessageSquareText, show: isSuperAdmin },
          ...
          <div className="p-4 border-b border-border bg-muted/30">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground capitalize">{user.role === "super_admin" ? "Super Administrador" : user.role}</p>
              </div>
            </div>
            <ThemeToggle />
            {availableDatabases.length > 0 && (
              <div className="mt-3">
                ...
                <Select value={activeDatabase ? String(activeDatabase.id) : undefined} onValueChange={handleDatabaseChange} disabled={setActiveDatabase.isPending}>
                  ...
                  <SelectContent>
                    {availableDatabases.map(database => <SelectItem key={database.id} value={String(database.id)}>{database.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
            )}
            ...
            {navigation.filter(item => item.show).map(item => {
              ...
              return (
                <Link key={item.name} href={item.href}>
                  <a className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${active ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`} onClick={() => setSidebarOpen(false)}>
