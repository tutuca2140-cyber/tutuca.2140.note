...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const originalTitleRef = useRef(
    typeof document !== "undefined" ? document.title : "Note Note"
  );
  ...
  const canWatchSupport = Boolean(
    isSuperAdmin || user?.adminCanSupport || commercialAccount
  );

  useEffect(() => {
    if (!user || !canWatchSupport) {
    ...
    const poll = async () => {
      try {
        const response = await fetch(
          "/api/site-access?scope=support&action=notifications",
        ...
        const from =
          data?.audience === "admin"
            ? data?.latest?.name || data?.latest?.username || "Cliente"
            : "Suporte Note Note";
        const preview = String(
          data?.latest?.message || "Nova mensagem recebida."
        ).slice(0, 110);
        toast.info(`Nova mensagem de suporte — ${from}`, {
          description: preview,
          duration: 8000,
        });
        if (
          ...
          const notification = new Notification(`Note Note — ${from}`, {
            body: preview,
            icon: "/brand/note-note-icon.png",
            tag: `support-${latestId}`,
          });
          notification.onclick = () => {
            window.focus();
            if (data?.audience === "admin") navigate("/admin/suporte");
            notification.close();
          };
        }
      } catch {
  ...
  useEffect(() => {
    ...
    document.title =
      supportUnreadCount > 0
        ? `(${supportUnreadCount}) ${originalTitleRef.current}`
  ...
  const navigation: NavItem[] = [
    ...
    },
    {
      ...
      href: "/clientes",
      icon: Users,
      show: Boolean(user?.canView && regularAccess),
    },
    {
      name: "Empréstimos",
      href: "/emprestimos",
    ...
    },
    {
      ...
      href: "/relatorios",
      icon: FileText,
      show: Boolean(user?.canGenerateReports && regularAccess),
    },
    {
      name: "Agentes",
      href: "/agentes",
    ...
    },
    {
      ...
      href: "/meu-banco",
      icon: Database,
      show: Boolean(canManageOwnDatabases && regularAccess),
    },
    {
      name: "Equipe",
      href: "/equipe",
      icon: UserRoundCog,
      show: Boolean(canManageTeam && regularAccess),
    },
  ];
  const adminNavigation: NavItem[] = [
    {
      ...
      href: "/admin/controle",
      icon: Activity,
      show: Boolean(isSuperAdmin || user?.adminCanControlPanel),
    },
    {
    ...
    },
    {
      ...
      icon: CreditCard,
      show: Boolean(isSuperAdmin || user?.adminCanSubscriptions),
    },
    {
      name: "Marketing",
      href: "/admin/marketing",
      icon: Mail,
      show: Boolean(isSuperAdmin || user?.adminCanMarketing),
    },
    {
      name: "Suporte",
      href: "/admin/suporte",
      icon: MessageCircle,
      show: Boolean(isSuperAdmin || user?.adminCanSupport),
    },
    {
      name: "Avaliações",
    ...
    },
    {
      ...
      href: "/admin/usuarios",
      icon: Shield,
      show: Boolean(isSuperAdmin || user?.adminCanUsers),
    },
    {
      name: "Bancos de Dados",
      href: "/admin/bancos",
      icon: Database,
      show: Boolean(isSuperAdmin || user?.adminCanDatabases),
    },
    {
      name: "Auditoria",
      href: "/admin/auditoria",
      icon: FileText,
      show: Boolean(isSuperAdmin || user?.adminCanAudit),
    },
    {
      name: "Configurações",
      href: "/admin/configuracoes",
      icon: Settings,
      show: Boolean(user?.canAccessSettings),
    },
  ];
    ...
    return (
      <Link key={item.name} href={item.href}>
        ...
        >
          ...
          {item.href === "/admin/suporte" && supportUnreadCount > 0 ? (
            <span
              className={`min-w-5 rounded-full px-1.5 py-0.5 text-center text-[10px] font-black ${active ? "bg-primary-foreground text-primary" : "bg-primary text-primary-foreground"}`}
            >
    ...
    <div className="app-shell min-h-screen bg-muted/30">
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 h-14 border-b border-border/70 bg-background/90 backdrop-blur-xl supports-[backdrop-filter]:bg-background/75">
        <div className="flex h-full items-center justify-between px-3">
          ...
          <Link href="/perfil">
            <a
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary ring-1 ring-primary/10"
      ...
      <aside
        className={`fixed top-0 left-0 z-[60] h-[100dvh] w-[min(86vw,320px)] bg-background border-r border-border shadow-2xl transition-transform duration-200 ease-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:z-40 lg:h-screen lg:w-64 lg:translate-x-0 lg:shadow-none`}
      >
        <div className="flex h-full flex-col">
              ...
              <div className="mt-2 lg:mt-3">
                ...
                {isSuperAdmin ? (
                  <p className="mb-1.5 text-[10px] leading-relaxed text-muted-foreground">
                    Bancos de clientes ficam protegidos em Super Admin → Bancos
                    de Dados.
                  </p>
                ) : null}
                <Select
          ...
          <nav className="flex-1 overflow-y-auto overscroll-contain px-3 py-3 space-y-0.5 lg:py-4 lg:space-y-1">
            ...
            {(isSuperAdmin ||
              user?.adminCanControlPanel ||
              user?.adminCanSubscriptions ||
              user?.adminCanMarketing ||
              user?.adminCanSupport ||
              user?.adminCanUsers ||
              user?.adminCanDatabases ||
              user?.adminCanAudit ||
          ...
          <div className="border-t border-border p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] lg:p-4">
            <Button
              variant="ghost"
              className="h-10 w-full justify-start rounded-xl text-destructive hover:text-destructive hover:bg-destructive/10"
      ...
      <nav className="mobile-app-nav lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-border/70 bg-background/92 px-2 pt-1.5 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80">
        <div className="mx-auto grid max-w-md grid-cols-5 items-end gap-1 pb-[max(0.35rem,env(safe-area-inset-bottom))]">
          {mobileQuickNavigation.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}>
                <a
                  className={`flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-xl px-1 text-[10px] font-semibold transition-all active:scale-95 ${active ? "text-primary" : "text-muted-foreground"}`}
                >
                  <span
                    className={`flex h-7 w-10 items-center justify-center rounded-full transition-colors ${active ? "bg-primary/12" : ""}`}
                  >
          ...
          {Array.from({
            length: Math.max(0, 4 - mobileQuickNavigation.length),
          }).map((_, index) => (
          ...
          <button
            ...
            className={`flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-xl px-1 text-[10px] font-semibold transition-all active:scale-95 ${sidebarOpen ? "text-primary" : "text-muted-foreground"}`}
          >
            <span
              className={`flex h-7 w-10 items-center justify-center rounded-full ${sidebarOpen ? "bg-primary/12" : ""}`}
            >
