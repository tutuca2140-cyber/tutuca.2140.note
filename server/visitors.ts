...
function maskIp(value: unknown) {
  ...
  return parts.length===4?`${parts[0]}.${parts[1]}.•••.•••`:"IP protegido";
}
function device(agent: unknown){const ua=String(agent||"");const type=/mobile|android|iphone/i.test(ua)?"Celular":/tablet|ipad/i.test(ua)?"Tablet":"Computador";const browser=/edg/i.test(ua)?"Edge":/firefox/i.test(ua)?"Firefox":/chrome|crios/i.test(ua)?"Chrome":/safari/i.test(ua)?"Safari":"Outro";return {type,browser};}
function source(value:unknown){const ref=String(value||"");if(!ref)return "Acesso direto";try{const host=new URL(ref).hostname.replace(/^www\./,"");if(host.includes("google"))return "Google";if(host.includes("instagram"))return "Instagram";if(host.includes("facebook"))return "Facebook";return host;}catch{return "Outro site";}}

export async function handleVisitors(req:any,res:any){
  if(req.method!=="GET"){res.setHeader("Allow","GET");return sendJson(res,405,{success:false,message:"Método não permitido."});}
  try{
    const admin=await getAuthorizedAdmin(req,"control");
    if(!admin)return sendJson(res,403,{success:false,message:"Acesso restrito ao Super Admin."});
    const sql=getSql();
    ...
    const visitors=rows.map((row:any,index:number)=>{const info=device(row.userAgent);const minutes=Math.max(0,Math.round((new Date(row.lastSeen).getTime()-new Date(row.firstSeen).getTime())/60000));return {...row,label:row.name||row.username||`Visitante #${String(row.visitorId).replace(/-/g,"").slice(-6).toUpperCase()}`,anonymous:!row.name&&!row.username&&!row.email,converted:Boolean(row.name||row.username||row.email),maskedIp:maskIp(row.ipAddress),device:info.type,browser:info.browser,source:source(row.referrer),durationMinutes:minutes,rank:index+1,ipAddress:undefined,userAgent:undefined};});
    return sendJson(res,200,{success:true,visitors,summary:{total:visitors.length,anonymous:visitors.filter((v:any)=>v.anonymous).length,converted:visitors.filter((v:any)=>v.converted).length,online:visitors.filter((v:any)=>Date.now()-new Date(v.lastSeen).getTime()<10*60*1000).length}});
  }catch(error){console.error("[visitors]",error);return sendJson(res,500,{success:false,message:error instanceof Error?error.message:"Não foi possível carregar os visitantes."});}
}
