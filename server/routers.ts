...
const stripLegacyCpf = <T extends { cpf?: unknown }>(
  client: T
): Omit<T, "cpf"> => {
...
const protectedProcedure = baseProtectedProcedure.use(({ ctx, next, path }) => {
  ...
  return db.withUserDatabaseScope(
...
export const appRouter = router({
  ...
  auth: router({
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!passwordMatch) {
          if (
            ...
            user.username?.toLowerCase() === "draco"
          ) {
            throw new TRPCError({
              code: "UNAUTHORIZED",
              message: "Usuário ou senha inválidos",
    ...
    requestPasswordReset: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const user =
          (await db.getUserByUsername(input.identifier)) ??
          (await db.getUserByEmail(input.identifier));
    ...
    resetPassword: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!user || user.username === "Draco") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Esta conta não pode usar recuperação de senha.",
          });
        }
    ...
    create: superAdminProcedure
      .input(
        z.object({
          ...
          canAdminControl: z.boolean().default(false),
          canAdminSubscriptions: z.boolean().default(false),
          canAdminMarketing: z.boolean().default(false),
          canAdminSupport: z.boolean().default(false),
          canAdminDatabases: z.boolean().default(false),
          canAdminAudit: z.boolean().default(false),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!createdUser)
          throw new TRPCError({
            ...
            message: "Não foi possível confirmar o usuário criado.",
          });
    ...
    update: superAdminProcedure
      .input(
        z.object({
          ...
          canAdminControl: z.boolean(),
          canAdminSubscriptions: z.boolean(),
          canAdminMarketing: z.boolean(),
          canAdminSupport: z.boolean(),
          canAdminDatabases: z.boolean(),
          canAdminAudit: z.boolean(),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (target.username?.toLowerCase() === "draco")
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "O Super Admin protegido não pode ser editado.",
          });
        ...
        await db.createAuditLog({
          ...
          entity: "users",
          entityId: userId,
          details: `Usuário editado: ${input.username}`,
          status: "success",
        });
        return { success: true };
      }),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (targetUser?.username === "Draco") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message:
              "As permissões do super administrador Draco são imutáveis.",
          });
        }
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (targetUser?.username === "Draco") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message:
              "O super administrador Draco não pode ter o perfil alterado.",
          });
        }
    ...
    toggleActive: superAdminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (targetUser?.username === "Draco") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "O super administrador Draco não pode ser desativado.",
          });
        }
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (targetUser.username === "Draco") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message:
              "A senha do super administrador Draco não pode ser alterada.",
          });
        }
    ...
    delete: superAdminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (targetUser?.username === "Draco") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "O super administrador Draco não pode ser excluído.",
          });
        }
  ...
  agents: router({
    list: protectedProcedure
      .input(
        z
          .object({ includeInactive: z.boolean().default(true).optional() })
          .optional()
      )
      .query(async ({ input, ctx }) => {
        if (!ctx.user.canView) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para visualizar agentes.",
          });
        }
        const activeDb = await db.getActiveDatabase();
        if (!activeDb) return [];
        return await db.getAgentsByDatabase(
          activeDb.id,
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user.canInsert) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para inserir agentes.",
          });
        }
        const activeDb = await db.getActiveDatabase();
        if (!activeDb)
        ...
        const result = await db.createAgent({
          ...
          defaultCommissionPercentage:
            input.defaultCommissionPercentage.toFixed(2),
          status: "ACTIVE",
          createdBy: ctx.user.id,
        });
        await db.createAuditLog({
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user.canEdit) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para editar agentes.",
          });
        }
        const activeDb = await db.getActiveDatabase();
        const targetAgent = activeDb
          ? await db.getAgentById(input.id)
          : undefined;
        if (
          !activeDb ||
          !targetAgent ||
          targetAgent.databaseId !== activeDb.id
        ) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Agente não encontrado no banco ativo.",
          });
        }
        ...
        await db.updateAgent(
          id,
          {
            ...
            ...(defaultCommissionPercentage === undefined
              ...
              : {
                  defaultCommissionPercentage:
                    defaultCommissionPercentage.toFixed(2),
                }),
          },
          activeDb.id
        );
    ...
    deactivate: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user.canEdit) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para desativar agentes.",
          });
        }
        const activeDb = await db.getActiveDatabase();
        const targetAgent = activeDb
    ...
    history: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        if (!ctx.user.canView) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para visualizar o histórico.",
          });
        }
        const activeDb = await db.getActiveDatabase();
        if (!activeDb)
  ...
  clients: router({
    ...
    profile: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (!profile)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Cliente não encontrado no banco ativo.",
          });
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db.updateClientInDatabase(
          id,
          {
            ...data,
            ...(birthDate !== undefined
  ...
  loans: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      if (!ctx.user.canView)
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Você não tem permissão para visualizar empréstimos.",
        });
      const activeDb = await db.getActiveDatabase();
      if (!activeDb) return [];
    ...
    details: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        if (!ctx.user.canView)
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para visualizar detalhes.",
          });
        const activeDb = await db.getActiveDatabase();
        if (!activeDb) return null;
        const loan = await db.getLoanById(input.id);
    ...
    generateInterest: protectedProcedure
      .input(
        z.object({
          ...
          periodReference: z.string().trim().min(1).max(20),
        })
      )
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user.canEdit)
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para lançar juros.",
          });
        const activeDb = await db.getActiveDatabase();
        if (!activeDb)
          throw new TRPCError({
        ...
        if (!loan || loan.databaseId !== activeDb.id)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Empréstimo não encontrado no banco ativo.",
          });
        if (loan.status === "pago" || loan.status === "cancelado")
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Não é possível lançar juros em um empréstimo encerrado.",
          });
        if (
          await db.getLoanInterestPeriod(
            input.loanId,
            activeDb.id,
          ...
          throw new TRPCError({
            code: "CONFLICT",
            message: "Os juros deste período já foram lançados.",
          });
        }
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Valor principal ou taxa de juros inválidos.",
          });
        }
        ...
        const endDate = input.endDate
          ...
          : addPeriods(startDate, plan.periods, ratePeriod);
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Datas do empréstimo inválidas.",
          });
        const principal =
        ...
        const ratePercent =
          input.interestRate !== undefined
            ...
            : Number(currentLoan.interestRate);
        if (
          !Number.isFinite(principal) ||
          principal <= 0 ||
          !Number.isFinite(ratePercent) ||
        ...
        const installmentAmount =
          input.installmentAmount === undefined
            ? Number(currentLoan.installmentAmount)
            : Number(input.installmentAmount);
        if (!Number.isFinite(installmentAmount) || installmentAmount <= 0)
          throw new TRPCError({
            code: "BAD_REQUEST",
        ...
        await db.updateLoanInDatabase(
          ...
          financialTermsChanged || hasInitialInterest
            ? initialInterest
            : undefined
        );
        await db.createAuditLog({
  ...
  payments: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (
          ...
          (input.vehicleFinancingId === undefined)
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Informe exatamente um empréstimo ou um financiamento.",
          });
        }
        ...
        const loan =
          input.loanId === undefined
            ? undefined
            : await db.getLoanById(input.loanId);
        ...
        if (
          input.loanId !== undefined &&
          (!loan ||
            loan.databaseId !== activeDb.id ||
            ["pago", "cancelado"].includes(loan.status))
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Empréstimo inválido, encerrado ou fora do banco ativo.",
          });
        }
        if (
          input.vehicleFinancingId !== undefined &&
          (!vehicleFinancing ||
            vehicleFinancing.databaseId !== activeDb.id ||
            ["pago", "cancelado"].includes(vehicleFinancing.status))
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Financiamento inválido, encerrado ou fora do banco ativo.",
          });
        }
        ...
        if (
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "As datas do pagamento são inválidas.",
          });
        ...
        if (input.agentId !== undefined) {
          ...
          if (!agent || agent.databaseId !== activeDb.id)
            throw new TRPCError({
              code: "BAD_REQUEST",
              message: "Agente inválido para o banco ativo.",
            });
          if (agent.status !== "ACTIVE")
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Agentes inativos não podem ser selecionados em novos pagamentos.",
            });
          agentId = agent.id;
          commissionPercentage =
            input.commissionPercentage ??
            Number(agent.defaultCommissionPercentage || 0);
        } else if (
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "A comissão só pode ser informada quando um agente é selecionado.",
          });
        }
        ...
        if (vehicleFinancing && input.status === "pago") {
          ...
          if (
            priorPayments.some(
              payment =>
                payment.status === "pago" &&
                payment.installmentNumber === input.installmentNumber
            )
          ) {
            throw new TRPCError({
              code: "CONFLICT",
              message: `A cota ${input.installmentNumber} já foi paga.`,
            });
          }
        }
        const contractPrincipal = loan
          ? Number(loan.principalBalance || loan.amount)
          : Number(vehicleFinancing?.financedAmount || 0);
        const accruedInterest = loan
          ? Number(loan.accruedInterest || 0)
          : Math.max(
              0,
              Number(
                vehicleFinancing?.totalAmount ||
                  vehicleFinancing?.financedAmount ||
                  0
              ) - contractPrincipal
            );
            ...
            ? loan
              ...
              : (() => {
                  ...
                  const installmentInterest = roundMoney(
                    Math.max(
                      0,
                      Number(vehicleFinancing!.totalAmount) -
                        Number(vehicleFinancing!.financedAmount)
                  ...
                  return {
                    ...
                    principalAmount: roundMoney(
                      Math.max(0, regularQuota - interestAmount) +
                        extraAmortization
                    ),
                    remainingBalance: roundMoney(
        ...
        const commissionAmount =
          Math.round(paymentAmount * commissionPercentage) / 100;
        const netAmount =
          Math.round((paymentAmount - commissionAmount) * 100) / 100;
        ...
        if (duplicate)
          throw new TRPCError({
            code: "CONFLICT",
            message: "Este pagamento e sua comissão já foram registrados.",
          });
        ...
        const nextPrincipal = roundMoney(
          Math.max(
            0,
            Number(loan?.principalBalance || loan?.amount || 0) -
        ...
        const result = await db.createPaymentBundle(
          paymentData,
          {
            databaseId: activeDb.id,
            type: "ENTRADA",
            ...
            description: loan
              ? `Recebimento do empréstimo #${loan.id}`
              : `Recebimento do financiamento #${vehicleFinancing!.id}`,
            amount: paymentAmount.toFixed(2),
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!current || current.databaseId !== activeDb.id)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Pagamento não encontrado no banco ativo.",
          });
        const amount =
          input.amount !== undefined
            ? Number(input.amount)
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        return {
          success: true,
          message: "Pagamento excluído e caixa reconciliado.",
          result,
        };
      }),
  }),
  ...
  cashFlow: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      if (!ctx.user.canView)
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Você não tem permissão para visualizar o fluxo de caixa.",
        });
      const activeDb = await db.getActiveDatabase();
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user.canInsert)
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para registrar movimentações.",
          });
        const activeDb = await db.getActiveDatabase();
        if (!activeDb)
          throw new TRPCError({
  ...
  vehicles: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const { result } = await db.createVehicleBundle(
          vehicleData,
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!currentVehicle || currentVehicle.databaseId !== activeDb.id)
          throw new TRPCError({
            code: "NOT_FOUND",
            message: "Veículo não encontrado no banco ativo.",
          });
        ...
        const normalizedData = {
          ...
          expenses:
            data.expenses === undefined ? undefined : data.expenses.toFixed(2),
          salePrice:
  ...
  vehicleSales: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      if (!ctx.user.canView)
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Você não tem permissão para visualizar vendas.",
        });
      const activeDb = await db.getActiveDatabase();
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const result = await db.createVehicleSaleBundle(
          data,
          input.vehicleId,
          activeDb.id,
          {
    ...
    receive: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user.canInsert)
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para registrar recebimentos.",
          });
        const activeDb = await db.getActiveDatabase();
        if (!activeDb)
          throw new TRPCError({
        ...
        if (Number.isNaN(movementDate.getTime()))
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Data do recebimento inválida.",
          });
        const result = await db.receiveVehicleSaleBundle(
          input.saleId,
          activeDb.id,
  ...
  vehicleFinancings: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (downPayment >= vehiclePrice) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "A entrada não pode ser maior que o preço do veículo.",
          });
        }
    ...
    update: protectedProcedure
      .input(
        z.object({
          id: z.number(),
          vehiclePrice: positiveDecimal("Preço do veículo").optional(),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const { id } = input;
        const vehiclePrice =
          input.vehiclePrice === undefined
            ? Number(financing.vehiclePrice)
            : Number(input.vehiclePrice);
        const downPayment =
          input.downPayment === undefined
            ? Number(financing.downPayment)
            : Number(input.downPayment);
        if (downPayment >= vehiclePrice)
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "A entrada deve ser menor que o preço do veículo.",
          });
        const interestRate =
          input.interestRate === undefined
            ? Number(financing.interestRate)
            : Number(input.interestRate);
    ...
    agentPerformance: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        if (!ctx.user.canView) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não tem permissão para visualizar performance.",
          });
        }
        const activeDb = await db.getActiveDatabase();
        if (!activeDb) {
