...
import { handleCommercialProfile } from "./profile-service.js";
import {
...
function assertDelegationAllowed(
  ...
  for (const key of permissionKeys) {
    if (permissions[key] && !Boolean(viewer[key])) {
      throw Object.assign(
        new Error(
          `Você não pode conceder a permissão ${key} porque ela não está liberada para sua conta.`
        ),
        { statusCode: 403 }
      );
    }
...
async function validateDatabaseIds(ownerId: number, databaseIds: number[]) {
  const uniqueIds = Array.from(
    new Set(databaseIds.filter(id => Number.isInteger(id) && id > 0))
  );
  ...
  if (uniqueIds.some(id => !allowed.has(id))) {
    throw Object.assign(
      new Error("Só é possível liberar bancos pertencentes ao contratante."),
      { statusCode: 403 }
    );
  }
  return uniqueIds;
...
async function handleTeam(req: any, res: any, viewer: any) {
  ...
  if (action === "create") {
    ...
    try {
      ...
      await sql`DELETE FROM user_database_access WHERE "userId" = ${member.id}`;
      await sql`DELETE FROM users WHERE id = ${member.id} AND "loginMethod" = 'commercial_subuser'`;
      throw error;
    }
  ...
  if (action === "delete") {
    ...
    await writeTeamAudit(viewer, "delete_commercial_subuser", userId, {
      ownerId: account.ownerId,
      username: target.username,
    });
    return sendJson(res, 200, {
      success: true,
      message: "Usuário removido da conta.",
    });
  }
  ...
  if (action === "update") {
    ...
    const previousDatabaseIds = await sql`
      SELECT "databaseId" FROM user_database_access WHERE "userId" = ${userId}
    `;
    await sql`DELETE FROM user_database_access WHERE "userId" = ${userId}`;
    try {
      for (let index = 0; index < databaseIds.length; index++) {
    ...
    await writeTeamAudit(viewer, "update_commercial_subuser", userId, {
      ownerId: account.ownerId,
      username,
      databaseIds,
      permissions,
    ...
    return sendJson(res, 200, {
      success: true,
      message: "Usuário e permissões atualizados.",
    });
  }
...
async function handleCashDelete(req: any, res: any, viewer: any) {
  ...
  if (!Number.isInteger(id) || id <= 0 || reason.length < 3) {
    return sendJson(res, 400, {
      success: false,
      message:
        "Informe um lançamento válido e uma observação com pelo menos 3 caracteres.",
    });
  }
  ...
  if (viewer.role !== "super_admin") {
    const access = await sql`
      ...
      WHERE "userId" = ${viewer.id} AND "databaseId" = ${movement.databaseId}
      LIMIT 1
    `;
    if (!access[0]) {
      return sendJson(res, 403, {
        success: false,
        message: "Você não tem acesso a este banco.",
      });
    }

    const isCommercialOwner =
      viewer.loginMethod === "commercial_signup" &&
      Number(movement.createdBy) === Number(viewer.id);
    const isAuthorizedSubuser =
      viewer.loginMethod === "commercial_subuser" &&
      Number(viewer.accountOwnerId) === Number(movement.createdBy) &&
      Boolean(viewer.canDeleteCashFlow);

    if (!isCommercialOwner && !isAuthorizedSubuser) {
      return sendJson(res, 403, {
        success: false,
        message:
          "O contratante não liberou permissão para apagar lançamentos do caixa.",
      });
    }
  }
...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (scope === "profile") return await handleCommercialProfile(req, res);
    ...
    return sendJson(res, 400, {
      success: false,
      message: "Área comercial não reconhecida.",
    });
  } catch (error: any) {
    console.error("[commercial-account]", error);
