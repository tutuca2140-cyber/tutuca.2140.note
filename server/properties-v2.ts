...
function ensureTables(){
  ...
  ensurePromise=(async()=>{
    ...
    await sql`ALTER TABLE property_rentals ADD COLUMN IF NOT EXISTS "agentId" integer`;
    await sql`ALTER TABLE property_rentals ADD COLUMN IF NOT EXISTS "commissionPercentage" numeric(5,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_financings ADD COLUMN IF NOT EXISTS "agentId" integer`;
    await sql`ALTER TABLE property_financings ADD COLUMN IF NOT EXISTS "commissionPercentage" numeric(5,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_sales ADD COLUMN IF NOT EXISTS "agentId" integer`;
    await sql`ALTER TABLE property_sales ADD COLUMN IF NOT EXISTS "commissionPercentage" numeric(5,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_sales ADD COLUMN IF NOT EXISTS "commissionAmount" numeric(15,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_sales ADD COLUMN IF NOT EXISTS "netAmount" numeric(15,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_rental_payments ADD COLUMN IF NOT EXISTS "agentId" integer`;
    await sql`ALTER TABLE property_rental_payments ADD COLUMN IF NOT EXISTS "commissionPercentage" numeric(5,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_rental_payments ADD COLUMN IF NOT EXISTS "commissionAmount" numeric(15,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_rental_payments ADD COLUMN IF NOT EXISTS "netAmount" numeric(15,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_financing_payments ADD COLUMN IF NOT EXISTS "agentId" integer`;
    await sql`ALTER TABLE property_financing_payments ADD COLUMN IF NOT EXISTS "commissionPercentage" numeric(5,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_financing_payments ADD COLUMN IF NOT EXISTS "commissionAmount" numeric(15,2) DEFAULT 0 NOT NULL`;
    await sql`ALTER TABLE property_financing_payments ADD COLUMN IF NOT EXISTS "netAmount" numeric(15,2) DEFAULT 0 NOT NULL`;
...
function validCommission(v:any){const n=num(v)||0;return Number.isFinite(n)&&n>=0&&n<=100?n:null;}
async function validateAgent(databaseId:number,agentId:number|null){if(!agentId)return null;const sql=getSql();const rows=await sql`SELECT id,name,"defaultCommissionPercentage" FROM agents WHERE id=${agentId} AND "databaseId"=${databaseId} AND status='ACTIVE' LIMIT 1`;return (rows[0] as any)||null;}
...
function financingFromRate(principal:number,rate:number,installments:number){const total=principal+(principal*rate/100)*installments;return {rate,total,installmentAmount:total/installments};}
function financingFromInstallment(principal:number,installmentAmount:number,installments:number){const total=installmentAmount*installments;const rate=principal>0&&installments>0?Math.max(0,((total/principal)-1)*100/installments):0;return {rate,total,installmentAmount};}

async function listAll(databaseId:number){const sql=getSql();const [properties,rentals,financings,clients,payments,financingPayments,sales,agents]=await Promise.all([
  sql`SELECT * FROM properties WHERE "databaseId"=${databaseId} ORDER BY "createdAt" DESC`,
  sql`SELECT r.*,p.title AS "propertyTitle",p.type AS "propertyType",p.address AS "propertyAddress",c.name AS "clientName",c.phone AS "clientPhone",c.whatsapp AS "clientWhatsapp",a.name AS "agentName" FROM property_rentals r JOIN properties p ON p.id=r."propertyId" JOIN clients c ON c.id=r."clientId" LEFT JOIN agents a ON a.id=r."agentId" WHERE r."databaseId"=${databaseId} ORDER BY r."createdAt" DESC`,
  sql`SELECT f.*,p.title AS "propertyTitle",p.type AS "propertyType",c.name AS "clientName",a.name AS "agentName",COALESCE((SELECT SUM(fp.amount) FROM property_financing_payments fp WHERE fp."financingId"=f.id),0) AS "totalPaid" FROM property_financings f JOIN properties p ON p.id=f."propertyId" JOIN clients c ON c.id=f."clientId" LEFT JOIN agents a ON a.id=f."agentId" WHERE f."databaseId"=${databaseId} ORDER BY f."createdAt" DESC`,
  sql`SELECT id,name,phone,whatsapp,email FROM clients WHERE "databaseId"=${databaseId} ORDER BY name`,
  sql`SELECT rp.*,r."propertyId",r."clientId",p.title AS "propertyTitle",c.name AS "clientName",a.name AS "agentName" FROM property_rental_payments rp JOIN property_rentals r ON r.id=rp."rentalId" JOIN properties p ON p.id=r."propertyId" JOIN clients c ON c.id=r."clientId" LEFT JOIN agents a ON a.id=rp."agentId" WHERE rp."databaseId"=${databaseId} ORDER BY rp."dueDate" DESC`,
  sql`SELECT fp.*,f."propertyId",f."clientId",p.title AS "propertyTitle",c.name AS "clientName",a.name AS "agentName" FROM property_financing_payments fp JOIN property_financings f ON f.id=fp."financingId" JOIN properties p ON p.id=f."propertyId" JOIN clients c ON c.id=f."clientId" LEFT JOIN agents a ON a.id=fp."agentId" WHERE fp."databaseId"=${databaseId} ORDER BY fp."paymentDate" DESC`,
  sql`SELECT s.*,p.title AS "propertyTitle",p.type AS "propertyType",c.name AS "clientName",a.name AS "agentName" FROM property_sales s JOIN properties p ON p.id=s."propertyId" JOIN clients c ON c.id=s."clientId" LEFT JOIN agents a ON a.id=s."agentId" WHERE s."databaseId"=${databaseId} ORDER BY s."saleDate" DESC`,
  sql`SELECT id,name,"defaultCommissionPercentage",status FROM agents WHERE "databaseId"=${databaseId} AND status='ACTIVE' ORDER BY name`
]);return {properties,rentals,financings,clients,payments,financingPayments,sales,agents,currentMonth:new Date().toISOString().slice(0,7),propertyTypes:PROPERTY_TYPES};}
...
export async function handlePropertiesV2(req:any,res:any){
  try{
    ...
    if(action==="pay_rent"){
      requirePerm(user,"canEdit");const rentalId=Math.trunc(num(body.rentalId)),reference=text(body.referenceMonth,7);if(!/^\d{4}-\d{2}$/.test(reference))return sendJson(res,400,{success:false,message:"Mês de referência inválido."});const rent=await sql`SELECT r.*,p.title,c.name AS "clientName" FROM property_rentals r JOIN properties p ON p.id=r."propertyId" JOIN clients c ON c.id=r."clientId" WHERE r.id=${rentalId} AND r."databaseId"=${databaseId} LIMIT 1`;const r=rent[0] as any;if(!r)return sendJson(res,404,{success:false,message:"Aluguel não encontrado."});const existing=await sql`SELECT id FROM property_rental_payments WHERE "rentalId"=${rentalId} AND "referenceMonth"=${reference} LIMIT 1`;if(existing[0])return sendJson(res,409,{success:false,message:"Este aluguel já foi pago neste mês."});const [year,month]=reference.split("-").map(Number),lastDay=new Date(year,month,0).getDate(),dueDate=`${reference}-${String(Math.min(Number(r.dueDay),lastDay)).padStart(2,"0")}`,amount=num(body.amount)||num(r.monthlyRent),commissionPercentage=Number(r.commissionPercentage||0),commissionAmount=amount*commissionPercentage/100,netAmount=amount-commissionAmount;const cashFlowId=await cashEntry(databaseId,amount,"ALUGUEL_IMOVEL",`Aluguel ${reference} - ${r.title} - ${r.clientName}`,Number(r.clientId),user.id);await sql`INSERT INTO property_rental_payments ("databaseId","rentalId","referenceMonth",amount,"dueDate","paymentDate",status,"cashFlowId","agentId","commissionPercentage","commissionAmount","netAmount","createdBy") VALUES (${databaseId},${rentalId},${reference},${amount},${dueDate},NOW(),'pago',${cashFlowId},${r.agentId||null},${commissionPercentage},${commissionAmount},${netAmount},${user.id})`;return sendJson(res,200,{success:true,commissionAmount,netAmount});
    }
    if(action==="create_financing"){
      requirePerm(user,"canInsert");const propertyId=Math.trunc(num(body.propertyId)),clientId=Math.trunc(num(body.clientId)),salePrice=num(body.salePrice),down=num(body.downPayment)||0,installments=Math.trunc(num(body.installments)),startDate=text(body.startDate,10),agentId=body.agentId?Math.trunc(num(body.agentId)):null;const rateProvided=body.interestRate!==""&&body.interestRate!=null;const installmentProvided=body.installmentAmount!==""&&body.installmentAmount!=null;const requestedRate=rateProvided?num(body.interestRate):NaN,requestedInstallment=installmentProvided?num(body.installmentAmount):NaN;const agent=await validateAgent(databaseId,agentId);if(agentId&&!agent)return sendJson(res,400,{success:false,message:"Agente comissionado inválido ou inativo."});const commission=validCommission(body.commissionPercentage??agent?.defaultCommissionPercentage);if(commission===null)return sendJson(res,400,{success:false,message:"A comissão deve estar entre 0% e 100%."});if(!propertyId||!clientId||salePrice<=0||down<0||down>=salePrice||installments<1||!startDate)return sendJson(res,400,{success:false,message:"Dados do financiamento imobiliário inválidos."});if(!rateProvided&&!installmentProvided)return sendJson(res,400,{success:false,message:"Informe a taxa de juros ou o valor desejado da parcela."});const principal=salePrice-down;let calculation;if(rateProvided){if(!Number.isFinite(requestedRate)||requestedRate<0)return sendJson(res,400,{success:false,message:"Informe uma taxa mensal válida."});calculation=financingFromRate(principal,requestedRate,installments);}else{if(!Number.isFinite(requestedInstallment)||requestedInstallment<=0||requestedInstallment*installments<principal)return sendJson(res,400,{success:false,message:"O valor da parcela é insuficiente para o principal financiado."});calculation=financingFromInstallment(principal,requestedInstallment,installments);}const prop=await sql`SELECT id,title,type,status FROM properties WHERE id=${propertyId} AND "databaseId"=${databaseId} LIMIT 1`;const p=prop[0] as any;if(!p)return sendJson(res,404,{success:false,message:"Imóvel não encontrado."});if(String(p.status)!=="disponivel")return sendJson(res,409,{success:false,message:"Este imóvel não está disponível para financiamento."});const rows=await sql`INSERT INTO property_financings ("databaseId","propertyId","clientId","salePrice","downPayment","interestRate",installments,"installmentAmount","totalAmount","startDate","agentId","commissionPercentage",notes,"createdBy") VALUES (${databaseId},${propertyId},${clientId},${salePrice},${down},${calculation.rate},${installments},${calculation.installmentAmount},${calculation.total},${startDate},${agentId},${commission},${text(body.notes,4000)||null},${user.id}) RETURNING *`;await sql`UPDATE properties SET status='financiado',"salePrice"=${salePrice},"updatedAt"=NOW() WHERE id=${propertyId}`;if(down>0)await cashEntry(databaseId,down,"ENTRADA_FINANCIAMENTO_IMOVEL",`Entrada financiamento - ${p.title}`,clientId,user.id);return sendJson(res,201,{success:true,financing:rows[0],calculatedInterestRate:calculation.rate,calculatedInstallmentAmount:calculation.installmentAmount});
    }
    if(action==="cancel_financing"){
      requirePerm(user,"canEdit");const financingId=Math.trunc(num(body.financingId));const rows=await sql`SELECT id,"propertyId",status FROM property_financings WHERE id=${financingId} AND "databaseId"=${databaseId} LIMIT 1`;const f=rows[0] as any;if(!f)return sendJson(res,404,{success:false,message:"Financiamento não encontrado."});if(String(f.status)!=="ativo")return sendJson(res,409,{success:false,message:"Somente financiamentos ativos podem ser cancelados."});await sql`UPDATE property_financings SET status='cancelado',"updatedAt"=NOW() WHERE id=${financingId}`;await sql`UPDATE properties SET status='disponivel',"updatedAt"=NOW() WHERE id=${Number(f.propertyId)} AND "databaseId"=${databaseId}`;return sendJson(res,200,{success:true});
    }
    if(action==="delete_financing"){
      requirePerm(user,"canDelete");const financingId=Math.trunc(num(body.financingId));const rows=await sql`SELECT f.id,f."propertyId",f."clientId",f."downPayment",f.status,f."createdBy",f."createdAt",p.title AS "propertyTitle" FROM property_financings f JOIN properties p ON p.id=f."propertyId" WHERE f.id=${financingId} AND f."databaseId"=${databaseId} LIMIT 1`;const f=rows[0] as any;if(!f)return sendJson(res,404,{success:false,message:"Financiamento não encontrado."});const paymentRows=await sql`SELECT "cashFlowId" FROM property_financing_payments WHERE "financingId"=${financingId}`;const hasFinancialMovement=Number(f.downPayment||0)>0||paymentRows.length>0;if(hasFinancialMovement&&user.role!=="super_admin")return sendJson(res,409,{success:false,message:"Este financiamento possui movimentação financeira. Somente o Super Admin pode excluí-lo definitivamente; você pode cancelá-lo para preservar o histórico."});if(user.role==="super_admin"){for(const row of paymentRows as any[]){const cashId=Number(row.cashFlowId||0);if(cashId>0)await sql`DELETE FROM cash_flow WHERE id=${cashId} AND "databaseId"=${databaseId}`;}if(Number(f.downPayment||0)>0){const desc=`Entrada financiamento - ${String(f.propertyTitle)}`;await sql`DELETE FROM cash_flow WHERE id IN (SELECT id FROM cash_flow WHERE "databaseId"=${databaseId} AND category='ENTRADA_FINANCIAMENTO_IMOVEL' AND description=${desc} AND "clientId"=${Number(f.clientId)} AND amount=${Number(f.downPayment)} AND "createdBy"=${Number(f.createdBy)} ORDER BY ABS(EXTRACT(EPOCH FROM ("movementDate"-${new Date(f.createdAt)}))) ASC LIMIT 1)`;}}await sql`DELETE FROM property_financings WHERE id=${financingId} AND "databaseId"=${databaseId}`;await sql`UPDATE properties SET status='disponivel',"updatedAt"=NOW() WHERE id=${Number(f.propertyId)} AND "databaseId"=${databaseId}`;return sendJson(res,200,{success:true});
    }
    if(action==="pay_financing"){
      requirePerm(user,"canEdit");const financingId=Math.trunc(num(body.financingId)),installmentNumber=Math.trunc(num(body.installmentNumber));const rows=await sql`SELECT f.*,p.title,c.name AS "clientName" FROM property_financings f JOIN properties p ON p.id=f."propertyId" JOIN clients c ON c.id=f."clientId" WHERE f.id=${financingId} AND f."databaseId"=${databaseId} LIMIT 1`;const f=rows[0] as any;if(!f)return sendJson(res,404,{success:false,message:"Financiamento não encontrado."});if(String(f.status)!=="ativo")return sendJson(res,409,{success:false,message:"Este financiamento não está ativo."});if(installmentNumber<1||installmentNumber>Number(f.installments))return sendJson(res,400,{success:false,message:"Número de parcela inválido."});const existing=await sql`SELECT id FROM property_financing_payments WHERE "financingId"=${financingId} AND "installmentNumber"=${installmentNumber} LIMIT 1`;if(existing[0])return sendJson(res,409,{success:false,message:"Esta parcela já foi paga."});const amount=num(body.amount)||num(f.installmentAmount),dueDate=installmentDue(f.startDate,installmentNumber),commissionPercentage=Number(f.commissionPercentage||0),commissionAmount=amount*commissionPercentage/100,netAmount=amount-commissionAmount;const cashFlowId=await cashEntry(databaseId,amount,"PARCELA_FINANCIAMENTO_IMOVEL",`Parcela ${installmentNumber}/${f.installments} - ${f.title} - ${f.clientName}`,Number(f.clientId),user.id);await sql`INSERT INTO property_financing_payments ("databaseId","financingId","installmentNumber",amount,"dueDate","cashFlowId","agentId","commissionPercentage","commissionAmount","netAmount","createdBy") VALUES (${databaseId},${financingId},${installmentNumber},${amount},${dueDate},${cashFlowId},${f.agentId||null},${commissionPercentage},${commissionAmount},${netAmount},${user.id})`;const count=await sql`SELECT COUNT(*)::int AS total FROM property_financing_payments WHERE "financingId"=${financingId}`;if(Number((count[0] as any)?.total)>=Number(f.installments)){await sql`UPDATE property_financings SET status='pago',"updatedAt"=NOW() WHERE id=${financingId}`;await sql`UPDATE properties SET status='vendido',"updatedAt"=NOW() WHERE id=${Number(f.propertyId)}`;}return sendJson(res,200,{success:true,commissionAmount,netAmount});
    }
    if(action==="sell_cash"){
      requirePerm(user,"canInsert");const propertyId=Math.trunc(num(body.propertyId)),clientId=Math.trunc(num(body.clientId)),amount=num(body.amount),agentId=body.agentId?Math.trunc(num(body.agentId)):null;const agent=await validateAgent(databaseId,agentId);if(agentId&&!agent)return sendJson(res,400,{success:false,message:"Agente comissionado inválido ou inativo."});const commissionPercentage=validCommission(body.commissionPercentage??agent?.defaultCommissionPercentage);if(commissionPercentage===null)return sendJson(res,400,{success:false,message:"A comissão deve estar entre 0% e 100%."});if(!propertyId||!clientId||amount<=0)return sendJson(res,400,{success:false,message:"Informe imóvel, cliente e valor da venda."});const prop=await sql`SELECT title,status FROM properties WHERE id=${propertyId} AND "databaseId"=${databaseId} LIMIT 1`;const p=prop[0] as any;if(!p)return sendJson(res,404,{success:false,message:"Imóvel não encontrado."});if(String(p.status)!=="disponivel")return sendJson(res,409,{success:false,message:"Este imóvel não está disponível para venda."});const commissionAmount=amount*commissionPercentage/100,netAmount=amount-commissionAmount,cashFlowId=await cashEntry(databaseId,amount,"VENDA_IMOVEL",`Venda à vista - ${p.title}`,clientId,user.id);await sql`INSERT INTO property_sales ("databaseId","propertyId","clientId",amount,"cashFlowId","agentId","commissionPercentage","commissionAmount","netAmount",notes,"createdBy") VALUES (${databaseId},${propertyId},${clientId},${amount},${cashFlowId},${agentId},${commissionPercentage},${commissionAmount},${netAmount},${text(body.notes,4000)||null},${user.id})`;await sql`UPDATE properties SET status='vendido',"salePrice"=${amount},"updatedAt"=NOW() WHERE id=${propertyId}`;return sendJson(res,201,{success:true,commissionAmount,netAmount});
    }
